# Guides

**Type:** Documentation
**Last Updated:** 2026-04-07
**Files:** 14

## Documentation Files

| File | Title | Description | Lines |
|------|-------|-------------|-------|
| [README.md](README.md) | Agent Guides | - agent-8-automation.md - Quick start + script index (start  | 62 |
| [agent-6-comprehensive-onboarding.md](agent-6-comprehensive-onboarding.md) |  | Your mission: Build and maintain production-ready Streamlit  | 528 |
| [agent-6-quick-start.md](agent-6-quick-start.md) |  | You build the **Streamlit web interface** for the structural | 96 |
| [agent-8-automation.md](agent-8-automation.md) |  | bash ./scripts/agent_start.sh        # Unified setup | 338 |
| [agent-8-git-ops.md](agent-8-git-ops.md) |  | - ✅ **Workflow Enforcement** - Single entrypoint, eliminate  | 1316 |
| [agent-8-mistakes-prevention-guide.md](agent-8-mistakes-prevention-guide.md) |  | <!-- lint-ignore-git --> > ⚠️ **HISTORICAL DOCUMENTATION NOT | 1123 |
| [agent-8-multi-agent-coordination.md](agent-8-multi-agent-coordination.md) |  | Agent 8 workflow (ai_commit.sh + safe_push.sh) now supports  | 336 |
| [agent-8-operations-log-spec.md](agent-8-operations-log-spec.md) |  |  | 46 |
| [agent-9-governance-hub.md](agent-9-governance-hub.md) |  | Agent 9 manages **governance, documentation structure, and m | 146 |
| [agent-9-quick-start.md](agent-9-quick-start.md) |  | Agent 9 is the **governance and documentation structure agen | 277 |
| [agent-automation-system.md](agent-automation-system.md) |  | Comprehensive automation system for error-free AI agent work | 363 |
| [agent-bootstrap-complete-review.md](agent-bootstrap-complete-review.md) |  | docs/getting-started/agent-bootstrap.md is the 60-second onb | 125 |
| [agent-quick-reference.md](agent-quick-reference.md) |  | | Need | Guide | Use When | |------|-------|----------| | 345 |
| [agent-workflow-master-guide.md](agent-workflow-master-guide.md) |  | > **Deep reference guide.** Start with agent-bootstrap.md fi | 712 |
