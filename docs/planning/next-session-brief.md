...
| **API first** | The owner's first step is reusable API knowledge. `ETABS-API-GUIDE` supplies `./run.sh etabs-api search`, `workflow` and `show`, with all installed interface methods, version-matched help and existing source owners. Read the [integration guide](../guides/etabs-api-integration.md); discovery and live qualification are separate. |
| **Next** | After the API foundation, start `BEAM-PROVISIONAL-REVIEW` from the [prepared U1–U5 execution card](xll-product/etabs-design-workflow.md#prepared-next-milestone-persistent-assumptions-and-provisional-review--2026-09-18): one persistent field resolver, independently labelled core/provisional results, automatic Excel dispatch/edit refresh and complete review persistence. R01–R12 define the functional milestone; runtime implementation remains pending. |
