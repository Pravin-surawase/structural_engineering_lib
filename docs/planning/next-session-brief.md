...
- Recurrence controls: RR-002 x18 / unknown: Freeze acceptance rows and run an exact-host micro-probe before building the host adapter or package.; RR-005 x61 / unknown: Use maintained launchers, literal PowerShell blocks, exact paths, native output tools, and one shell per operation.; RR-013 x3 / unknown: Declare LF explicitly for maintained .cs, .csproj, and .slnx files and renormalize the full candidate diff.
<!-- HANDOFF:END -->
