...
<!-- HANDOFF:START -->
- Date: 2026-09-27
- Focus: Preserve imported depth through design and saved projects; use essential checks and multiple commits in one milestone PR.
- Completed: Distinguished exact D/d aliases while retaining duplicate/conflicting-header rejection and preventing a lone overall-depth column from supplying effective depth.; Implemented compact changed-area checks and explicit --full mode; added owner-directed scope changes on the same task/branch with retained timing and history. Required hosted gates and the three lightweight commit hooks remain.; Corrected the operation hook's partial-commit false hold; recognize Git's own next-index context while retaining other lock holds and preserving unrelated staged content.
- Recurrence controls: RR-049 x1 / unknown: Recognize the current worktree next-index only in operation-hook completion mode; preserve raw state, normal validation and unrelated-lock holds.; RR-048 x1 / unknown: Write a final LF in the maintained snapshot generator and verify regenerated bytes through unchanged integrity controls.; RR-005 x229 / unknown: Discover paths/commands first; use explicit cwd, pinned MTP --project/--filter-class and actual JSON envelopes. Await completion before reading accepted artifacts.
<!-- HANDOFF:END -->
