...
- Recurrence controls: RR-005 x151 / unknown: Use maintained launchers, literal PowerShell blocks, exact paths, native output tools, and one shell per operation.; RR-040 x1 / unknown: Classify known reads from maintained getter definitions and keep the remaining methods unclassified; metadata never expands invocation authority.; RR-004 x16 / unknown: Use canonical timed gate commands and admitted delivery transitions; finish location-independent session/handoff records before the candidate.
<!-- HANDOFF:END -->
