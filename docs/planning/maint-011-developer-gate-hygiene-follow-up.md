...
| M11-13 | A manually refreshed brief displayed the SHA-256 of the whole receipt file instead of the receipt's embedded local-state hash. Both are valid SHA-256 values, so visual review alone did not reveal the semantic mismatch. | The receipt has two identities: artifact bytes and `local_state_receipt_hash`; the handoff contract uses the latter. The session validator printed the correct embedded hash but did not compare it with the already-written brief line. | File SHA was `d6b5c825…`; embedded state hash and session output were `d5518052…`. | Generate handoff lines from the receipt object rather than copying hashes manually, and add a consistency check between the current brief path/hash/status and the selected receipt. |
