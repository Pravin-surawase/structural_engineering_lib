...
> owns the human-facing goal, six-phase order, phase outputs and definition of
> done. This document remains the detailed packet, safety-control and evidence
> register. When a safety boundary differs, the stricter boundary applies.
...
- PR #952 accepted the installed A1/C1 evidence and exact schema at merge
  `c29a468a7d6023e8be92feb534b9479977eac52f`. Getter-only A1 state/file
  preservation passed and the C1 export resolved all requested tables and
  fields. The comparison result epoch remains `BLOCKED`, so the installed
  values are diagnostic rather than fresh project comparison truth.
...
The owner-requested six-phase master-plan freeze is the one planning-only
alignment exception. After it, do not create a planning-only PR between these milestones. Update this plan,
...
Therefore the candidate-data milestone was narrowed to B1B/B2 and C2 remained
held until the separately authorized A1/C1 evidence was accepted. PR #952 now
satisfies that dependency, so C2 is the next offline implementation packet. It
must preserve the blocked result-epoch `HOLD`. This is a dependency correction,
not a return to one PR per task.
...
| 11 | `ETABS-W3-C1-INSTALLED-DESIGN-EXPORT-INVENTORY` | Acquire one target/runtime/epoch-bound design snapshot, named export manifest and actual SQLite schema inventory | A1 and C0 plus separate authorization | Every requested field/table closes a named row or is rejected; acquisition mode explicit; artifact complete/hash-bound; state/file preservation passes | accepted through PR #952 at `c29a468a`: 160 tables/62,133 rows, 10/10 requested tables and 80/80 fields found; comparison epoch remains `BLOCKED` |
| 12 | `ETABS-W3-C2-OFFLINE-EXACT-SCHEMA-PARSER` | Implement the parser and matched comparison against the acquired exact schema | accepted C1 artifact | Frozen export passes integrity/schema/row bounds; canonical evidence and diagnostic comparison are reproducible; no ETABS/UI call | **NEXT** — PR #952 accepted the exact schema; implement offline and retain the result-epoch `HOLD` for comparison values |
