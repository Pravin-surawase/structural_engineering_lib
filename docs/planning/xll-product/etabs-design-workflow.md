...
| T5 handoff and delivery | One allowed integrity repair | Searchable topology recipe, catalogue, sanitized receipt and corpus/next-agent plan are prepared. First candidate's fresh capture/replay passed; read-only integrity rejected mixed line endings in the recurrence JSON. Normalize that file, preserve all implementation bytes, and bind fresh evidence to the repaired candidate. Broad cumulative gate remains C0c. | Reconcile #997 before candidate, refresh affected docs/format evidence, audit exact candidate, run integrity once and complete required hosted delivery. |
