...
| Session log | Process mistake | The WP09 entry omitted the explicit `**Completed:**` list. | Second closeout repair. | Complete the newest entry before `PREPARED`; final `session end` is strictly read-only. |
| Hosted integrity | Missing local control | Two lock files and one rollback receipt lacked final newlines. | First hosted run failed; normalization changed evidence; second candidate and hosted run were required. | Run changed-path formatting before the candidate and the read-only integrity check once after independent acceptance. |
...
7. finish session/handoff records and run `./run.sh check
   --candidate-integrity` once on the already accepted immutable candidate;
