...
4. If releasing: `./scripts/ci_local.sh` then `.venv/bin/.venv/bin/python scripts/verify_release.py --version X.Y.Z --source pypi`
...
- Local wheel (pre-release): `.venv/bin/.venv/bin/python scripts/verify_release.py --source wheel --wheel-dir Python/dist`
- PyPI (post-release): `.venv/bin/.venv/bin/python scripts/verify_release.py --version X.Y.Z --source pypi`
...
1. Run: `.venv/bin/.venv/bin/python scripts/end_session.py --fix`
...
1. **Collect diagnostics bundle:**
   ```bash
   .venv/bin/.venv/bin/python scripts/collect_diagnostics.py > diagnostics.txt
   ```
...
4. **Run validators:**
   ```bash
   .venv/bin/.venv/bin/python scripts/generate_api_manifest.py --check
   .venv/bin/.venv/bin/python scripts/check_scripts_index.py
