...
| 2026-08-29 | ETABS-EXCEL-BEAM-W2C-INSTALLED-ACCEPTANCE | After the separately authorized exact selection prerequisite, reconciled accepted direct/REST baseline `d4c28586...` and repaired only volatile observation-time hash drift; installed Excel wrote six exact tables but failed at JSON `Range.values` on `+`/`-`-prefixed chunks, so preserved model state and froze `BLOCKED_SAFE_EXCEL_JSON_WRITE` without a second repair | pending candidate |
| 2026-08-29 | ETABS-EXCEL-BEAM-W2C-TRANSACTIONAL-ACCEPTANCE | Added typed literal JSON cells and rollback-safe seven-table transactions, normalized Excel blank/15-digit readback, then reconciled direct/REST/all saved Excel tables and exact canonical JSON while preserving the copied model; bounded software acceptance passed pending Mac review | pending candidate |
