# Modern Python Tooling Evaluation — structural_engineering_lib
...
Modern Python tooling has **dramatically evolved** in the past 2-3 years. Tools like `uv`, `Hypothesis`, `pytest-benchmark`, and `mutmut` represent the **cutting edge** of Python development practices. This research evaluates their fit for `structural_engineering_lib` — a deterministic calculation library with strict correctness requirements.

### Key Findings

**High-Value Tools (Adopt Immediately):**
- ✅ **uv** — 10-100x faster than pip, reproducible builds, zero-config lock files
- ✅ **Hypothesis** — Property-based testing catches edge cases human testers miss

**Medium-Value Tools (Adopt Selectively):**
- 🟡 **pytest-benchmark** — Performance regression tracking (useful but not critical)

**Low-Value Tools (Defer):**
- 🟢 **mutmut** — Mutation testing (advanced, time-intensive, diminishing returns)
...
## 1. uv — Modern Dependency Manager
...
| **Lock files** | ❌ | ✅ | ✅ |
| **Resolver quality** | ⚠️ OK | ✅ Good | ✅ **Excellent** |
...
- No lock files → different developers get different dependency versions
- Dependency conflicts resolved non-deterministically

**How uv Solves These:**
- ✅ CI installs reduced to <5 seconds (10x faster)
- ✅ Lock files guarantee exact versions across machines/CI
- ✅ Better resolver catches conflicts earlier
...
**That's it!** It's a drop-in replacement — same CLI, same behavior, just faster.
...
**Benefits:**
- ✅ Every developer gets exact same dependency versions
- ✅ CI uses exact same versions as local dev
- ✅ Upgrading dependencies is explicit (regenerate lock file)

### Pros & Cons for structural_lib

**Pros:**
- ✅ **10-100x faster** — CI time reduced by 50%+
- ✅ **Reproducible builds** — Lock files guarantee exact versions
- ✅ **Better resolver** — Catches dependency conflicts earlier
- ✅ **Drop-in replacement** — Minimal migration effort
- ✅ **Active development** — Astral (ruff authors) maintaining
- ✅ **Industry momentum** — Major projects adopting (FastAPI, Pydantic)

**Cons:**
- 🟡 **Young project** — API may change (though stable since v0.1)
- 🟡 **One more tool** — Adds dependency (but pip remains fallback)
- 🟡 **Lock file maintenance** — Must regenerate when dependencies change

**Verdict:** ✅ **ADOPT IMMEDIATELY** (High value, low risk, minimal effort)
...
def test_mu_lim_properties(b, d, fck, fy):
    ...
    # Monotonicity: larger section → larger capacity
...
**Pros:**
- ✅ **Finds edge cases** — Catches bugs humans miss
- ✅ **Shrinks failures** — Reports minimal failing example
- ✅ **Documents properties** — Tests are also specifications
- ✅ **Mature library** — 10+ years development, battle-tested
- ✅ **Good for calculations** — Perfect fit for deterministic math
- ✅ **Minimal changes** — Runs alongside existing tests

**Cons:**
- 🟡 **Learning curve** — New mental model (properties vs. examples)
- 🟡 **Slower tests** — 100+ examples per test (configurable)
- 🟡 **Non-deterministic** — Different runs test different inputs (can be seeded)
- 🟡 **Not a replacement** — Complements, doesn't replace, unit tests

**Verdict:** ✅ **ADOPT FOR CORE FUNCTIONS** (High value, medium effort)
...
## 3. pytest-benchmark — Performance Regression Testing
...
**Use Cases:**
1. **Track optimization impact** — Prove cost optimization is actually faster
2. **Prevent regressions** — Alert if code changes slow down critical paths
3. **Identify bottlenecks** — Find functions worth optimizing
...
-------------------------------------------------------------------
test_benchmark_calculate_mu_lim     12.5µs  18.3µs  13.2µs  0.8µs
test_benchmark_design_beam_flexure  45.2µs  62.1µs  48.7µs  2.3µs
test_benchmark_design_beam_shear    38.9µs  54.6µs  41.2µs  1.9µs
test_benchmark_design_beam_is456   125.3µs 168.7µs 132.1µs  5.4µs
...
------------------------------------------------------------------------------------------
test_benchmark_design_beam_is456   152.3µs   132.1µs   +15.3% (REGRESSION!)
------------------------------------------------------------------------------------------
...
**Pros:**
- ✅ **Track optimization impact** — Prove cost optimizer is faster
- ✅ **Regression detection** — Alert on slowdowns
- ✅ **Statistical rigor** — Handles warmup, outliers automatically
- ✅ **Multiple formats** — JSON for CI, HTML for humans
- ✅ **Easy to add** — Just wrap function in `benchmark(func, ...)`

**Cons:**
- 🟡 **Noisy results** — CI runners vary in speed
- 🟡 **Time overhead** — Benchmarks take longer than unit tests
- 🟡 **Limited value** — Most functions are already fast (<1ms)
- 🟡 **Maintenance** — Baselines need updating when algorithm changes

**Verdict:** 🟡 **ADOPT SELECTIVELY** (Medium value, low effort)

**When to use:**
- ✅ Optimizing algorithms (e.g., cost optimizer, rebar cutting)
- ✅ Profiling bottlenecks before optimization
- ❌ Core calculations (already sub-millisecond, not bottleneck)
...
1. **Mutate:** Change one line of code (e.g., `if x > 0:` → `if x >= 0:`)
...
**Don't add to CI** — mutation testing is SLOW (minutes to hours).
...
**Pros:**
- ✅ **Finds test gaps** — Reveals untested edge cases
- ✅ **Validates test quality** — High mutation score = high confidence
- ✅ **Objective metric** — Quantifies test suite effectiveness

**Cons:**
- 🔴 **VERY SLOW** — 10-100x slower than regular tests
- 🔴 **Noisy results** — Many survivors are false positives
- 🔴 **High maintenance** — Requires manual review of survivors
- 🔴 **Diminishing returns** — 90% → 95% takes 10x effort of 70% → 80%

**Verdict:** 🟢 **DEFER TO LATER** (Advanced, low priority, high effort)

**When to use:**
- ✅ Quarterly audit of critical modules
- ✅ After major refactoring (validate tests still effective)
- ❌ Regular CI (too slow)
- ❌ Before mature test suite (low signal-to-noise)
...
|------|-------|----------------|-----|------|---------|
| **uv** | ✅ Instant | ✅ Minimal | 🔥 HIGH | ✅ Low | **ADOPT NOW** |
| **Hypothesis** | 🟡 Slower tests | 🟡 Medium | 🔥 HIGH | ✅ Low | **ADOPT FOR CORE** |
| **pytest-benchmark** | 🟡 Slow | ✅ Minimal | 🟡 MEDIUM | ✅ Low | **ADOPT SELECTIVELY** |
| **mutmut** | 🔴 VERY SLOW | 🔴 High | 🟡 MEDIUM | 🟡 Medium | **DEFER** |
...
### Week 1: uv Setup (4-6 hours) — 🔴 CRITICAL

**Goal:** Faster CI, reproducible builds

**Tasks:**
1. ✅ Install uv locally, test commands
2. ✅ Generate `requirements.lock` from `pyproject.toml`
3. ✅ Update CI workflows to use uv
4. ✅ Update `docs/contributing/development-guide.md`
5. ✅ Test full CI pipeline with uv

**Deliverables:**
- `Python/requirements.lock` committed
- CI install time reduced from 45s → 5s
...
### Week 2: Hypothesis Property Tests (6-8 hours) — 🔴 HIGH VALUE

**Goal:** Catch edge cases in core calculations

**Tasks:**
1. ✅ Install Hypothesis, read docs
2. ✅ Write property tests for `flexure.py` (5-10 properties)
3. ✅ Write property tests for `shear.py` (3-5 properties)
4. ✅ Write property tests for `materials.py` (2-3 properties)
5. ✅ Add to CI (separate job, can be slower)
6. ✅ Document property test workflow
...
### Week 3: Document & Refine (2-3 hours) — 🟡 CLEANUP

**Goal:** Ensure tools are usable by others

**Tasks:**
1. ✅ Update `docs/contributing/testing-strategy.md`
2. ✅ Add "Modern Tooling" section to dev guide
3. ✅ Create quick reference card (when to use each tool)
...
### Week 4: pytest-benchmark (Optional) (2-3 hours) — 🟢 NICE-TO-HAVE

**Goal:** Track performance of optimization algorithms

**Tasks:**
1. ✅ Add benchmarks for `optimization.optimize_beam_cost()`
2. ✅ Add benchmarks for `rebar_optimizer` functions
3. ✅ Run locally to establish baselines
4. ✅ Add to weekly cron job (not every commit)
...
### Week 8+: mutmut Audit (4-6 hours) — 🟢 ADVANCED

**Goal:** Quarterly test quality audit

**Tasks:**
1. ✅ Run mutmut on `flexure.py` (30 min)
2. ✅ Review survivors, write missing tests (2-3 hrs)
3. ✅ Run mutmut on `shear.py` (30 min)
4. ✅ Review survivors, write missing tests (1-2 hrs)
5. ✅ Document mutation score in quarterly report
...
**Key differences:**
- `pip` → `uv pip` (everything else stays the same)
...
**Both tests run** — property test doesn't replace golden test, it complements it.
...
def test_benchmark_trivial(benchmark):
    result = benchmark(lambda: 1 + 1)  # 10ns — meaningless
```

```python
# Good: Only benchmark functions that take >1µs
def test_benchmark_real_work(benchmark):
    result = benchmark(calculate_mu_lim, 230, 450, 25, 415)  # 15µs — useful
```
...
|-------|------|-------------------|-------------------|
| **uv setup** | 4-6 hrs | ✅ 10x faster CI | ✅ Reproducible builds |
| **Hypothesis** | 6-8 hrs | ✅ Find 1-2 bugs | ✅ Ongoing edge case coverage |
| **pytest-benchmark** | 2-3 hrs | 🟡 Performance visibility | 🟡 Track optimization impact |
| **mutmut audit** | 4-6 hrs | 🟡 Find test gaps | 🟡 Quarterly quality metric |
| **Total** | **16-23 hrs** | | |
...
- Savings: 40s per CI run × 10 runs/day × 250 days = **277 hours/year**
...
- Benefit: Improve test coverage from 85% → 90% (marginal)
...
1. **✅ Adopt uv** — 10-100x faster CI, reproducible builds, 4-6 hours effort
2. **✅ Adopt Hypothesis for core functions** — Catch edge cases, 6-8 hours effort

### Future Considerations (Next Quarter)

3. **🟡 Consider pytest-benchmark** — If optimization work is planned, 2-3 hours
4. **🟢 Defer mutmut** — Advanced tool, best for quarterly audits only

### Key Principle

> **"Adopt tools that save more time than they cost."**

- **uv** saves 277 hours/year, costs 6 hours → **46x ROI**
- **Hypothesis** catches 1-2 bugs/quarter, costs 8 hours → **2-4x ROI**
...
- ✅ This research provides deep-dive on 4 modern tools
- ✅ Explains when/why to use each tool
- ✅ Provides implementation guides with code examples

**User: "any coding practices"**
- ✅ Property-based testing is a **best practice** for math libraries
- ✅ Lock files are **industry standard** for reproducible builds
- ✅ Benchmarking is **standard practice** for performance-critical code
