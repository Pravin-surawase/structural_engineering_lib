...
| ETABS-EXCEL-BEAM-W3A-DEMAND-CONTRACT | Freeze versioned load-pattern/case/combination, result-selection, same-row beam-action, scenario, envelope, and governing-reference contracts before design expansion | Main Agent | one bounded Mac contract packet; later installed getter evidence separate | P1 | 🟡 READY TO START — exact predecessor `f1873e7b...`; no ETABS opening, setters, analysis/design run, optimization, or professional claim in W3A |
