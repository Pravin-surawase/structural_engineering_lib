...
**Updated:** 2026-09-01 — G0 live-route gate is active from accepted preflight PR #944 with the seven-milestone publication cadence
...
| LIB-BEAM-S0-CHECK-TRUTH | Remove hidden effective-depth assumptions and make REST/WebSocket supplied-beam V2 semantics consume supported fields or fail/hold explicitly | Main Agent / sole Windows writer | accepted G0 PR #945 | P0 | 🟡 FROZEN LOCAL CANDIDATE on `codex/w3-public-beam-truth` from `f7da72bd` — strict V2 request/result owner, shared REST/WebSocket service, maintained Python/TypeScript/React clients, release parity, OpenAPI plus WebSocket snapshots and field migration agree; focused calculation/transport/client checks pass. One milestone quick/hook/PR cycle remains. |
| LIB-PRO-015-D0-D1-BEAM | Add exact-wheel documentation obligations and complete the beam facade reference/examples without changing calculation outcomes | Main Agent / sole Windows writer | BEAM-S0 semantic freeze inside the same milestone | P1 | 🟡 FROZEN LOCAL CANDIDATE — all nine beam facade operations carry complete enforced docstrings; generated reference and valid/invalid/FAIL/HOLD examples pass source-bound and exact-wheel replay. Column documentation remains outside this beam-only slice; one combined PR follows the milestone gate. |
| ETABS-W3-A0-OFFLINE-SESSION-GUARD | Freeze exact process/runtime/target, getter-only attachment, OS lease/STA broker, model/result freshness and crash-durable evidence through fakes | Main Agent / same Windows worktree lane | accepted BEAM-S0 head for single-writer ordering | P0 | 📋 PLANNED — hash-chained raw-before-decode ledger plus atomic verifier; no COM/application call; installed attachment remains A1. |
...
| ETABS-W3-B0-CANONICAL-PILOT | Delegate the legacy live pilot to the canonical signed row-bound beam audit | Main Agent / sole writer | A0; installed run unnecessary | P0 | 📋 PLANNED — positive/negative M3 preserve physical TOP/BOTTOM face; missing provenance holds. |
| ETABS-W3-B1A-PROJECT-CRITERIA-CATALOGUE | Freeze declared-before-generation project criteria, permitted existing ETABS beam properties, stock, schedule, scenarios, objectives and digests | Main Agent / sole writer | BEAM-S0, B0 and W3A demand contracts | P0 | 📋 PLANNED — actual project instance may remain held; authored fixtures cannot become project evidence. |
...
| ETABS-W3-C0-OFFLINE-ACQUISITION-CONTRACT | Freeze matched-design/export contracts and generic bounded fixtures without claiming an ETABS SQLite schema | Main Agent / sole writer | A0 | P1 | 📋 PLANNED — zero ETABS/UI call. |
