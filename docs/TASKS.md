...
| WORKFLOW-READINESS-CROSS-DEVICE | Repair portable startup, exact-byte validation overhead and shared terminal/gate guidance before W3H | Main Agent / sole Windows writer | bounded maintenance before next engineering packet | P1 | IN PROGRESS — source/runtime/hook readiness and timing regressions; preserve both devices and every retained lane; exact candidate integration required. |
