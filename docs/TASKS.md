...
**Updated:** 2026-08-30 — W3D is merged through PR #906; separate W3R Pareto shear-feasibility repair is locally accepted and entering normal review
...
| ETABS-W3-PARETO-SHEAR-REPAIR | Repair the public Pareto optimizer so nonzero shear participates in feasibility/utilization and unknown objectives fail closed | Main Agent | separate candidate after W3D | P1 | 🟢 LOCAL ACCEPTANCE — maintained IS 456 shear and explicit `asv_mm2` now govern feasibility; flexure/shear/stirrup utilization are separate, high shear fails closed, `vu_kn` changes Pareto membership, unknown objectives fail, and typed Python/FastAPI/React surfaces retain explicit torsion/serviceability/stirrup-cost/global-analysis holds. Focused 28 Python/FastAPI and 6 React tests, generated contracts, lint and build pass; normal packet gates/review remain. |
