...
| W3-BEAM-INTEGRATED-PLAN | Reconcile the merged W3 PID-safe plan with current public beam depth/check defects and freeze one dependency-aware execution sequence | Main Agent / read-only parallel reviewers, sole writer | merged #941 and current `origin/main` | P0 | 🔄 PLANNING — `docs/planning/w3-beam-professional-integrated-execution-plan.md`; no application/model operation; next implementation is `LIB-BEAM-S0-CHECK-TRUTH`. |
| LIB-BEAM-S0-CHECK-TRUTH | Remove hidden effective-depth assumptions and make REST/WebSocket supplied-beam check semantics consume supported fields or fail/hold explicitly | Main Agent / sole Windows writer | accepted integrated plan | P0 | 📋 PLANNED — preserve compatibility wrappers and canonical valid outcomes; keep ETABS/Excel closed and execute focused depth, supplied-check, signed-face, torsion/detailing and BBS regressions. |
| ETABS-W3-A0-OFFLINE-SESSION-GUARD | Freeze PID candidates, two-phase target confirmation, live-session versus file freshness, state restoration and raw-call records through fake adapters only | Main Agent / same Windows worktree lane | accepted `LIB-BEAM-S0-CHECK-TRUTH` head for single-writer ordering | P0 | 📋 PLANNED — no COM/application calls; installed PID attachment is the separately authorized A1 evidence packet. |
| ETABS-W3-A1-INSTALLED-READONLY-ACCEPTANCE | Prove PID-selected attachment, live model identity, freshness disposition and exact state preservation without mutating the attached user session | bounded Windows ETABS evidence owner | accepted A0 plus separate authorization | P0 | 📋 PLANNED — exact installed version and selected model must be shown; Excel is optional projection only; unknown/unsaved state cannot become hash-bound baseline proof. |
