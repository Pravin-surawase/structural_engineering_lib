...
| ETABS-EXCEL-BEAM-W3D-LIVE-CATALOGUE | Extract one complete live read-only catalogue and linked same-row demand snapshot, reconciling direct/REST hashes with unchanged model, lock, units and selection | bounded Windows evidence owner | one live acceptance after merged W3C, plus an evidence-backed repair only if a proved sentinel blocks | P1 | 🟡 REPAIR VALIDATION — first live attempt correctly blocked with no partial value because ETABS returned documented zero-state sentinel `None`; model identity stayed exact. Narrow `LinearStaticInitialConditionV1` repair accepts only blank/`None` as `ZERO_UNSTRESSED`, keeps real prior cases blocked, and passes 24 focused tests, Ruff/Mypy, API/docs controls and quick gate 10/10; no live rerun before separate merge |
