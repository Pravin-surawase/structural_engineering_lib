...
|----|------|-------|-----|----------|--------|
| ETABS-EXCEL-BEAM-W2-BASELINE | Freeze and implement the complete read-only beam/model topology, result-provenance, and independent-frame-analysis feasibility contract before any optimization or write-back | Main Agent | one local contract packet plus separate Windows acceptance | P1 | 🟡 W2A MERGED / STATIC AUDIT COMPLETE LOCALLY / WINDOWS CAMPAIGN IN PROGRESS — PR #896 merged reviewed W2A as `0f5c918e...`; exact installed ETABS 23.3.1 metadata proves all 18 getters plus `SetPresentUnits` with no outcome-changing adapter mismatch. Phase A checkpoint validation/push, W2B REST/Excel, then guarded read-only W2C remain; `HELD_NOT_SUPPORTED`, no-analysis/no-mutation, and professional-review holds retained |
