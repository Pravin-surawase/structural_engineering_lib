...
**Updated:** 2026-09-02 — offline A0/B0/B1A/C0 foundations are integrated for one milestone PR; installed A1/C1 remains separately authorized
...
| ETABS-W3-B0-CANONICAL-PILOT | Delegate the legacy live pilot to the canonical signed row-bound beam audit | Main Agent / sole writer | A0; installed run unnecessary | P0 | 🟡 INTEGRATED CANDIDATE — the compatibility adapter delegates same-row signed actions to `beam_audit.py`, maps positive/negative M3 to explicit physical faces and returns typed `HOLD` when provenance is absent. Legacy transport remains disabled/deprecated pending A1. |
| ETABS-W3-B1A-PROJECT-CRITERIA-CATALOGUE | Freeze declared-before-generation project criteria, permitted existing ETABS beam properties, stock, schedule, scenarios, objectives and digests | Main Agent / sole writer | BEAM-S0, B0 and W3A demand contracts | P0 | 🟡 INTEGRATED CANDIDATE — canonical criteria, permitted existing-property catalogue and bounded reinforcement schedule contracts bind chronology, separate grades, stock, costs, scenarios/objectives and digests. Actual project instance remains held; authored fixtures cannot become project evidence. |
| ETABS-W3-B1B-CANDIDATE-EVALUATOR | Make one signed, layer-aware supplied-beam evaluation the feasibility owner | Main Agent / sole writer | accepted B1A | P0 | 📋 PLANNED — derive effective depth from selected layer centroid, rerun strength and pass an independent composition check. |
| ETABS-W3-B2-OPTIMIZER-CONVERGENCE | Route direct/cost/Pareto through B1B with complete deterministic search semantics | Main Agent / sole writer | accepted B1B | P0 | 📋 PLANNED — incomplete budgets remain provisional and cannot claim optimum, Pareto completeness or infeasibility. |
| ETABS-W3-C0-OFFLINE-ACQUISITION-CONTRACT | Freeze matched-design/export contracts and generic bounded fixtures without claiming an ETABS SQLite schema | Main Agent / sole writer | A0 | P1 | 🟡 INTEGRATED CANDIDATE — matched concrete-design basis and create-new export manifest bind target/runtime/model/result epoch, requested comparison rows, bounds, hashes and equal pre/post state. Generic fixtures prove zero ETABS/UI/SQLite-parser call; actual schema remains C1/C2. |
...
| ETABS-W3-C2-OFFLINE-EXACT-SCHEMA-PARSER | Implement the allowlisted parser and diagnostic comparison against C1's exact accepted schema | Main Agent / sole writer | accepted C1 artifact | P1 | 📋 PLANNED — read-only/private-cache integrity and schema/row bounds; zero ETABS/UI call. |
| MAINT-COMMIT-GATE-CONSOLIDATION | Move complete candidate assurance to one batched PR while reducing ordinary commits to mutation-safety guards | Main Agent / maintenance lane | accepted offline-foundations PR #947; no overlap with installed ETABS evidence | P1 | 🟡 INTEGRATED CANDIDATE — [implementation plan](planning/commit-pr-validation-consolidation-plan.md) is implemented as two logical commits for one batched PR: all 34 former hook capabilities have explicit hosted/guard/retirement ownership; ordinary commits run exactly 3 safety hooks, PR Repository Validation runs 8 manual integrity checks, and duplicate `push: main` validation is removed while strict no-bypass `PR Gate` remains the integration authority. Local focused/topology/manual-profile evidence passes; hosted acceptance is pending. |
