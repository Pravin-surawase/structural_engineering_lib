...
| ETABS-W3G-BEAM-LINE | Pure bounded beam-line solver and independent numerical references | Main Agent / sole Windows writer | W3F predecessor accepted; W3H remains separate | P1 | ✅ ACCEPTED — PR #915 merged as `3a21b626153dcfdb18873b51250f15b02e90328e`, tree `586d3f84ac09848fc48db8161bb2a455835d563e`; 62 focused tests and required hosted checks passed. No installed application access or calibration claim. |
| WORKFLOW-READINESS-CROSS-DEVICE | Repair portable startup, exact-byte validation overhead and shared terminal/gate guidance before W3H | Main Agent / sole Windows writer | bounded maintenance before next engineering packet | P1 | ACCEPTED — PR #916 merge `31fb89c23cd86137f3e20d3ea22aa1d5963a265a`, tree `7a5eac9ee6312fed3b1a6250d6ccaffb690bce31`; originating usage closed. Mac sync and cold speedup remain unproved. |
| ETABS-W3H-CALIBRATION | Exact model-specific beam-line reference comparison | Main Agent / one writer per packet | W3D/F/G accepted; project mapping/context/criteria required for L5 | P1 | L1 comparator accepted through #917; authored benchmarks accepted through #921. Actual-building L5 HELD. Saved mapping #929 and stopped diagnostics #931-#933 do not establish calibration. Correct zero-argument table call also returned CSI 1, disproving the placeholder theory and closing equivalent table API retries for this model/host. Continue saved-source reconciliation, suitability, proved direct getters and only named bounded export gaps. |
| ETABS-W3-WHOLE-PLAN-AUDIT | Audit all W3 dependencies, repeated issues and elapsed evidence; research installed/current CSI API and export alternatives; replace stale sequencing | Main Agent / sole Windows writer | documentation and saved/static evidence only, after #933 | P1 | ACCEPTED — PR #934 merge `f5e425ced924546d1f67d07f17841d0b83bfb68c`; whole-W3 source-first reset. Bounded reconciliation now follows below; no application or engineering acceptance. |
| ETABS-W3-SAVED-BASIS | Reconcile saved input definitions and loads; three candidate matrices; executable audit/criteria gap plan | Main Agent / sole Windows writer | accepted #934; saved bytes only | P1 | ACCEPTED in PR #935 as `349502d3`; saved investigation complete. 225 frame joins, five-case lineage, three NOT_COMPARABLE_AS_IS lines; cover/steel-basis hazards and W3E next packet specified. Actual-building H/I/K/L held. See `docs/planning/etabs-w3-saved-basis-and-criteria.md`. |
| ETABS-W3E-ROOT-CAUSE-FIXES | Correct traced reinforcement defects and preserve material/cover source meaning through real consumers | Main Agent / sole Windows writer | accepted #935; offline software | P1 | ACCEPTED: PR #936 merge `773d9673`, tree `f7d72d3c`; 716 Python, 101 FastAPI, 52 Node, types/architecture/quick/hooks/hosted gates. See `docs/planning/etabs-w3-reinforcement-root-causes.md`. |
| ETABS-W3E-SERVICEABILITY | Strict bounded span/depth and supplied Annex F checks through canonical Python/REST and W3 audit | Main Agent / sole Windows writer | accepted #936; offline software | P1 | ACCEPTED — PR #937 merge `690b2dad`, tree `9166afd6`; strict bounded serviceability and row-bound audit evidence integrated. Direct/long-term methods and actual-building evidence remain held. See `docs/planning/etabs-w3-serviceability.md`. |
| ETABS-W3E-TORSION-DETAILING | Consume verified torsion demands through source-correct longitudinal distribution, closed-stirrup limits and BBS | Main Agent / sole Windows writer | accepted #937/#938; offline software | P1 | ✅ ACCEPTED — PR #939 merged as `07d7c902`, tree `14c6cec1`; 221 focused Python, 68 API, configured types/architecture/docs/OpenAPI/client parity, quick, hooks and required hosted jobs passed. Supported b<=450 mm single-layer rectangular route; wider/multilayer/coupled cases fail closed. No ETABS/model call or project acceptance. |
| ETABS-W3E-SAVED-CANDIDATE-CONSUMPTION | Apply accepted strength/detail/BBS owners to the three saved candidate lines, repair signed-face loss, and freeze aggregate feasibility/hold evidence | Main Agent / sole Windows writer | accepted #939; saved bytes and offline software only | P1 | ✅ ACCEPTED — PR #940 merged as `c5357131`; 107/107 rows independently pass authored 75 mm and computed 75/100/125 mm strength/detailing/BBS feasibility after explicit TOP/BOTTOM projection. Legacy d'=40 blocks 107/107; frozen 150/200 spacing blocks detailing 107/107. Project service/applicability and W3H support/mesh/transfer gates remain held. |
| ETABS-W3-PROJECT-CRITERIA-RECOVERY | Audit current COM/session/optimizer owners and legacy VBA beam/column workflows; define the smallest PID-safe adapter, common candidate evaluator, ETABS design snapshot and owned-copy W3K/L transaction | Main Agent / sole Windows writer | accepted #940; static source/help audit only; separate from LIB-PRO-015 beam capability review | P1 | ✅ ACCEPTED — PR #941 merged as `5aad93b8`; the audit/plan records P0/P1/P2 functions, acceptance gates and effort ranges. No ETABS, COM, Excel, export, analysis/design or model operation occurred. |
| W3-BEAM-SIX-PHASE-MASTER-PLAN | Freeze one human-facing goal and six sequential phases from trusted ETABS acquisition through owned-copy reanalysis and convergence | Main Agent / sole writer | accepted A1/C1 PR #952 and owner phase decision | P0 | 🔄 ACTIVE — [master plan](planning/beam-design-optimization-master-plan.md) owns the phase order, outputs and exit gates. Phase 1 is the only active phase; later-phase code remains retained but cannot advance the programme. |
| W3-BEAM-INTEGRATED-PLAN | Reconcile the merged W3 PID-safe plan with current public beam depth/check defects and freeze one dependency-aware execution sequence | Main Agent / read-only parallel reviewers, sole writer | merged #941 and current `origin/main` | P0 | ✅ ACCEPTED — PR #942 merged as `35ea6b89`; no application/model operation. The long-term amendment below supersedes only future execution order. |
| W3-BEAM-LONG-TERM-PLAN-AMENDMENT | Audit runtime, public API/data, evaluator/search, solver, acquisition and owned-copy risks; update every maintained plan owner | Main Agent / three read-only subagents, sole writer | accepted PR #942 | P0 | ✅ ACCEPTED — PR #943 merged as `3b0d689d`; no ETABS, COM, Excel, export, solver, analysis/design or model call. |
| ETABS-LIVE-G0-ROUTE-GATE | Disable live routes by default and enforce loopback, HTTP/WebSocket auth and operation classification before any COM construction/attach | Main Agent / sole Windows writer | accepted preflight PR #944 | P0 | ✅ ACCEPTED — PR #945 reviewed candidate `aa87fdd03d6728031bf4547f63d835e255a6a98e` and merged as `f7da72bdf837a38e4c07ef6d7a21fb9267804ad5` with exact tree equality; required hosted checks passed. Default live paths remain absent and denied traffic creates no COM side effect. |
| LIB-BEAM-S0-CHECK-TRUTH | Remove hidden effective-depth assumptions and make REST/WebSocket supplied-beam V2 semantics consume supported fields or fail/hold explicitly | Main Agent / sole Windows writer | accepted G0 PR #945 | P0 | ✅ ACCEPTED — PR #946 merged as `f2a5c6f1`, tree `d96a58ed`; shared strict V2 Python/REST/WebSocket semantics, clients, migration evidence and exact-wheel replay passed. The successor audit repaired transverse-grade propagation and strict React result decoding before A0. |
| LIB-PRO-015-D0-D1-BEAM | Add exact-wheel documentation obligations and complete the beam facade reference/examples without changing calculation outcomes | Main Agent / sole Windows writer | BEAM-S0 semantic freeze inside the same milestone | P1 | ✅ ACCEPTED — PR #946 merged as `f2a5c6f1`, tree `d96a58ed`; all nine beam facade operations have generated complete reference/cookbook coverage and valid/invalid/FAIL/HOLD exact-wheel examples. Column documentation remains outside this beam-only slice. |
| ETABS-W3-A0-OFFLINE-SESSION-GUARD | Freeze exact process/runtime/target, getter-only attachment, OS lease/STA broker, model/result freshness and crash-durable evidence through fakes | Main Agent / same Windows worktree lane | accepted BEAM-S0 head for single-writer ordering | P0 | ✅ ACCEPTED — PR #947 merged as `16be0db`; OS-only discovery, runtime/session/checkpoint/freshness/epoch/capability contracts, getter-only state capture, fenced lease, supervised broker, raw-before-decode ledger and atomic evidence verifier pass offline failure injection. No COM/application call; installed attachment remains A1. |
| ETABS-W3-A1-INSTALLED-READONLY-ACCEPTANCE | Prove PID/start-time selected getter-only attachment, runtime/model identity and state/file preservation | bounded Windows ETABS evidence owner | accepted A0 plus separate authorization | P0 | ✅ ACCEPTED — PR #952 merged as `c29a468a`; exact PID/process/runtime/model attachment completed through the supervised getter-only broker. Pre/post state-content hashes are equal, restoration is `VERIFIED_EQUAL`, the model remained locked and its saved bytes were unchanged. The raw model-bearing evidence remains outside Git; non-proprietary proof is in `etabs-w3-a1-c1-installed-evidence.json`. |
| ETABS-W3-B0-CANONICAL-PILOT | Delegate the legacy live pilot to the canonical signed row-bound beam audit | Main Agent / sole writer | A0; installed run unnecessary | P0 | ✅ ACCEPTED — PR #947 merged as `16be0db`; the compatibility adapter delegates same-row signed actions to `beam_audit.py`, maps positive/negative M3 to explicit physical faces and returns typed `HOLD` when provenance is absent. Legacy transport remains disabled/deprecated pending A1. |
| ETABS-W3-B1A-PROJECT-CRITERIA-CATALOGUE | Freeze declared-before-generation project criteria, permitted existing ETABS beam properties, stock, schedule, scenarios, objectives and digests | Main Agent / sole writer | BEAM-S0, B0 and W3A demand contracts | P0 | ✅ ACCEPTED — PR #947 merged as `16be0db`; canonical criteria, permitted existing-property catalogue and bounded reinforcement schedule contracts bind chronology, separate grades, stock, costs, scenarios/objectives and digests. Actual project instance remains held; authored fixtures cannot become project evidence. |
| ETABS-W3-B1B-CANDIDATE-EVALUATOR | Make one signed, layer-aware supplied-beam evaluation the feasibility owner | Main Agent / sole writer | accepted B1A | P0 | ✅ COMPLETE — PR #950 merged at `742719dd`; the accepted batch binds a sealed signed row and exact schedule, recomputes layer-centroid depth, delegates strength/detailing to the maintained supplied-beam owner, independently recomputes quantities/cost, and maps missing mandatory evidence or authored fixtures to `HOLD`. |
| ETABS-W3-B2-OPTIMIZER-CONVERGENCE | Route direct/cost/Pareto through B1B with complete deterministic search semantics | Main Agent / sole writer | accepted B1B | P0 | ✅ COMPLETE — PR #950 merged at `742719dd`; hash-ordered finite traversal retains unchanged B1B evaluation identities across direct/cost/Pareto projections. Budget exhaustion and mandatory-check blocks remain provisional; infeasibility requires complete failed enumeration. |
| ETABS-W3-C0-OFFLINE-ACQUISITION-CONTRACT | Freeze matched-design/export contracts and generic bounded fixtures without claiming an ETABS SQLite schema | Main Agent / sole writer | A0 | P1 | ✅ ACCEPTED — PR #947 merged as `16be0db`; matched concrete-design basis and create-new export manifest bind target/runtime/model/result epoch, requested comparison rows, bounds, hashes and equal pre/post state. Generic fixtures prove zero ETABS/UI/SQLite-parser call; actual schema remains C1/C2. |
| ETABS-W3-C1-INSTALLED-DESIGN-EXPORT-INVENTORY | Acquire one target/runtime/result-epoch design basis, create-new export and actual schema inventory | bounded Windows ETABS evidence owner | A1/C0 plus separate authorization | P1 | ✅ ACCEPTED — PR #952 merged as `c29a468a`; one bounded in-memory concrete-design call, create-new UI SQLite export and getter-only cross-check completed without analysis/save/unlock/input/exit operations. Integrity is `ok`; 160 tables/62,133 rows, 10/10 requested tables and 80/80 fields resolve. The IS 456 beam summary has 3,502 rows, matching 3,502 API result items across 153/153 beams. Comparison values remain `HOLD` under the blocked result epoch. |
| ETABS-W3-C2-OFFLINE-EXACT-SCHEMA-PARSER | Implement the allowlisted parser and diagnostic comparison against C1's exact accepted schema | Main Agent / sole writer | accepted C1 artifact | P1 | 🔄 NEXT — PR #952 accepted the exact schema. Implement offline with zero ETABS/UI call and preserve the result-epoch `HOLD`; current exported values may not be interpreted as fresh comparison truth. |
| MAINT-COMMIT-GATE-CONSOLIDATION | Move complete candidate assurance to one batched PR while reducing ordinary commits to mutation-safety guards | Main Agent / maintenance lane | accepted offline-foundations PR #947; no overlap with installed ETABS evidence | P1 | ✅ ACCEPTED — PR #949 merged as `e6c684a5`, tree-equal to candidate `255d544f`; all 34 former hook capabilities have hosted/guard/retirement ownership, ordinary commits run exactly three mutation-safety hooks, comprehensive assurance runs once on the PR, and the duplicate main-push validation run is removed. |
| ETABS-W3-H0-ROUTE-DECISION | Choose `SURROGATE_ASSISTED`, `ETABS_FIRST` or terminal `HOLD` for the actual building | Main Agent / engineering evidence owner | B2, C2 and accepted criteria/catalogue instance | P0 | 📋 PLANNED — no new solver physics; baseline agreement cannot validate candidate stiffness range. |
| ETABS-W3-I-SCREENING | Produce the complete deterministic criteria-compliant shortlist under the H0 route | Main Agent / sole writer | B2/H0 | P0 | 📋 PLANNED — `SCREENED_ONLY`, no held mandatory checks and no changed-model feasibility claim. |
| ETABS-W3-K0-OFFLINE-TRANSACTION-KERNEL | Prove dry-run existing-beam-property changes, durable stages, result epoch and non-replaying recovery through fakes | Main Agent / sole writer | A0 and accepted W3I | P0 | 📋 PLANNED — columns/new property definitions unsupported; no COM/application call. |
| ETABS-W3-K1-INSTALLED-OWNED-COPY | Apply one exact candidate in an owned ETABS process/new local copy and rebuild fresh whole-model evidence | bounded Windows ETABS evidence owner | K0 plus separate mutation authorization | P0 | 📋 PLANNED — original/attached session unchanged; exact readback, safeguards and uninterrupted result epoch. |
| ETABS-W3-L-BOUNDED-ITERATION | Run finite clean-copy candidate cycles and independently repeat the accepted final candidate | Main Agent / Windows evidence owner | accepted K machinery and one complete K1 | P0 | 📋 PLANNED — freeze attempt/analysis/retry budgets; terminate with accepted repeat or explicit no-solution/budget/external/safety outcome. |
