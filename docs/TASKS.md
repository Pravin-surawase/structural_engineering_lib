...
**Updated:** 2026-09-02 — A1/C1 installed evidence is locally complete; C2 waits only for the single A1/C1 PR to be accepted
...
| ETABS-W3-C1-INSTALLED-DESIGN-EXPORT-INVENTORY | Acquire one target/runtime/result-epoch design basis, create-new export and actual schema inventory | bounded Windows ETABS evidence owner | A1/C0 plus separate authorization | P1 | 🟢 LOCALLY COMPLETE / PR PENDING — one bounded in-memory concrete-design call, create-new UI SQLite export and getter-only cross-check completed without analysis/save/unlock/input/exit operations. Integrity is `ok`; 160 tables/62,133 rows, 10/10 requested tables and 80/80 fields resolve. The IS 456 beam summary has 3,502 rows, matching 3,502 API result items across 153/153 beams. Exact schema is accepted for C2; comparison values remain `HOLD` under the blocked result epoch. |
| ETABS-W3-C2-OFFLINE-EXACT-SCHEMA-PARSER | Implement the allowlisted parser and diagnostic comparison against C1's exact accepted schema | Main Agent / sole writer | accepted C1 artifact | P1 | ⏸️ HELD UNTIL A1/C1 PR ACCEPTANCE — the exact local schema is now frozen and releases this dependency after the current batch merges. C2 remains offline with zero ETABS/UI call and must preserve the result-epoch `HOLD`; it may not interpret current exported values as fresh comparison truth. |
