...
**Updated:** 2026-09-02 — offline B1B/B2 convergence is accepted through PR #950; C2 remains held for separately authorized A1/C1 schema evidence
...
| ETABS-W3-A1-INSTALLED-READONLY-ACCEPTANCE | Prove PID/start-time selected getter-only attachment, runtime/model identity and state/file preservation | bounded Windows ETABS evidence owner | accepted A0 plus separate authorization | P0 | 🟡 IN PROGRESS / LIVE HOLD — exact-process no-COM preflight plus supervised `GetObjectProcess` getter transport, durable ledger and state/file bracket pass 19 focused fake cases with A0/C0. Installed ETABS 23.3.1/API 2.16.0 signatures are frozen, but no ETABS process/model is running; live attachment has zero calls and remains `HOLD` in `etabs-w3-a1-c1-preflight-hold-evidence.json`. |
...
| ETABS-W3-C1-INSTALLED-DESIGN-EXPORT-INVENTORY | Acquire one target/runtime/result-epoch design basis, create-new export and actual schema inventory | bounded Windows ETABS evidence owner | A1/C0 plus separate authorization | P1 | 🟡 IN PROGRESS / LIVE HOLD — read-only immutable SQLite integrity/schema/table/column/type/key/row-bound inventory and manifest binding pass seven C0/C1 focused cases. The real design basis and create-new operator export remain `HOLD` until A1 selects one exact running process/model; C1 still claims no parser support. |
| ETABS-W3-C2-OFFLINE-EXACT-SCHEMA-PARSER | Implement the allowlisted parser and diagnostic comparison against C1's exact accepted schema | Main Agent / sole writer | accepted C1 artifact | P1 | ⏸️ HELD — C0 intentionally contains no ETABS SQLite schema. Implement only after the separately authorized A1/C1 session freezes a complete hash-bound export and exact schema inventory; then run offline with zero ETABS/UI call. |
