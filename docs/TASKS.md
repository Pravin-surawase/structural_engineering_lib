...
| MAINT-COMMIT-GATE-CONSOLIDATION | Move complete candidate assurance to one batched PR while reducing ordinary commits to mutation-safety guards | Main Agent / maintenance lane | accepted offline-foundations PR #947; no overlap with installed ETABS evidence | P1 | 🟡 PLAN FROZEN — [implementation plan](planning/commit-pr-validation-consolidation-plan.md) maps all 34 hooks to hosted owners, 3 intentional commit guards or 1 warning-only retirement. Implement as two logical commits, one batched push and one PR; add PR parity before shrinking hooks, retain the live resolved-merge guard, and remove duplicate `push: main` validation only while strict no-bypass `PR Gate` remains proved. |
