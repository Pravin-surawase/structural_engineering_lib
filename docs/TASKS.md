...
|----|------|-------|-----|----------|--------|
| ETABS-EXCEL-BEAM-W2-BASELINE | Freeze and implement the complete read-only beam/model topology, result-provenance, and independent-frame-analysis feasibility contract before any optimization or write-back | Main Agent | one local contract packet plus separate Windows acceptance | P1 | 🟡 INTEGRATED / INSTALLED RETRY HELD — PR #897 merged the reviewed W2 candidate unchanged as exact `ee50aaa3...`. A separately authorized Windows retry repeated the approved copied-model/runtime preflight after the owner confirmed exact combination `117.(1.5DL+1.5LL)` active, but the authoritative getter returned inactive with zero selected combinations. The retry stopped before request construction with zero `FrameForce` calls/stations, no REST or Excel start, no setter, and unchanged model/workbook files. W2C therefore remains `BLOCKED_SAFE_NO_FORCE_READ`, not accepted; root cause of the confirmation/getter mismatch is unconfirmed. `HELD_NOT_SUPPORTED`, no-analysis/no-mutation, W3, and professional-review holds remain. |
