...
**Updated:** 2026-08-30 — W3C is accepted; W3D is preserving one blocked live sentinel observation while a narrow repair is validated
...
| ETABS-EXCEL-BEAM-W3D-LIVE-CATALOGUE | Extract one complete live read-only catalogue and linked same-row demand snapshot, reconciling direct/REST hashes with unchanged model, lock, units and selection | bounded Windows evidence owner | one live acceptance after merged W3C, plus an evidence-backed repair only if a proved sentinel blocks | P1 | 🟡 SECOND REPAIR VALIDATION — R1 merged in PR #904 as `e16870d0...`, tree `a6584729...`. One clean continuation passed the zero-state guard, then blocked without a partial catalogue on installed raw `Auto=5`; model identity stayed exact. R2 retains the raw integer, normalizes only documented 0/1, and marks any other Boolean interpretation `UNAVAILABLE`; 24 focused tests plus Ruff/Mypy pass and no retry occurred before separate review. |
