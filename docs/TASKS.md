...
| ETABS-W3-A1-INSTALLED-READONLY-ACCEPTANCE | Prove PID/start-time selected getter-only attachment, runtime/model identity and state/file preservation | bounded Windows ETABS evidence owner | accepted A0 plus separate authorization | P0 | 🟡 IN PROGRESS — `codex/w3-installed-readonly-evidence` started from current `origin/main`; installed runtime is present, but no ETABS process is running, so live attachment remains `HOLD`. Implement and fake-test the getter-only path without constructing COM until an exact PID/start/model is available. |
...
| ETABS-W3-C1-INSTALLED-DESIGN-EXPORT-INVENTORY | Acquire one target/runtime/result-epoch design basis, create-new export and actual schema inventory | bounded Windows ETABS evidence owner | A1/C0 plus separate authorization | P1 | 🟡 IN PROGRESS — build and fake-test the exact-schema inventory/binding offline; the installed create-new operator export remains `HOLD` until A1 selects one exact running process/model. No parser-support claim belongs to C1. |
