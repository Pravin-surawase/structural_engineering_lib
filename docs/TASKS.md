...
| ETABS-EXCEL-BEAM-W3F-FOUNDATION | Freeze model/topology/displacement/reaction contracts, prove installed getter signatures, normalize and accept separately bounded read-only evidence | Main Agent / Windows evidence | L1 then L2/L3, before W3G/H | P1 | 🟢 SPRING REPAIR — L1/L2 merged through PR #910. One live read blocked on unavailable spring assignments; unchanged-state diagnostic now confirms joint/frame scope. Explicit availability repair prepared from PR #912; exact-source live acceptance still pending. Stop before W3G. |
