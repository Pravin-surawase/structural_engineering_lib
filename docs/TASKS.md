...
|----|------|-------|-----|----------|--------|
| ETABS-EXCEL-BEAM-W2-BASELINE | Freeze and implement the complete read-only beam/model topology, result-provenance, and independent-frame-analysis feasibility contract before any optimization or write-back | Main Agent | one local contract packet plus separate Windows acceptance | P1 | 🟡 W2A WINDOWS PACKET/HANDOFF COMPLETE — `codex/etabs-excel-beam-w2a-baseline` contains the verified transport-neutral contract, setup portability repairs, machine-role/root-cause ledger, and Mac pickup plan; Windows stops writing after push; Mac review/PR/acceptance, W2B REST/Excel, and W2C installed acceptance are NOT STARTED; `HELD_NOT_SUPPORTED` retained |
