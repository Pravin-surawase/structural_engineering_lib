...
**Updated:** 2026-08-30 — W3D direct/REST live read-only catalogue and demand evidence is accepted locally; normal packet review is next
...
| ETABS-EXCEL-BEAM-W3D-LIVE-CATALOGUE | Extract one complete live read-only catalogue and linked same-row demand snapshot, reconciling direct/REST hashes with unchanged model, lock, units and selection | bounded Windows evidence owner | one live acceptance after merged W3C, plus evidence-backed narrow repairs for proved installed sentinels | P1 | 🟢 LOCAL ACCEPTANCE — R1/R2 merged through PRs #904/#905. Fresh merged source returned a complete 12-pattern/15-case/62-combination catalogue (`d44e6b89...`) and a 153-member/3,502-row demand snapshot (`7c1a4e21...`); direct and REST hashes match exactly. Model hash/size/mtime, lock, units, statuses and all 77 output-selection states stayed unchanged; no setter, `FrameForce`, analysis/design, save or Excel action occurred. Focused/API/docs/quick/hooks and hosted review remain the packet closeout. |
| ETABS-W3-PARETO-SHEAR-REPAIR | Repair the public Pareto optimizer so nonzero shear participates in feasibility/utilization and unknown objectives fail closed | Main Agent | separate candidate after W3D | P1 | ⏸ SEPARATE W3R — campaign authority is explicit; current path accepts `vu_kn` but marks flexure-only candidates safe, so Pareto remains unavailable for ETABS candidate selection until the separate repair merges |
