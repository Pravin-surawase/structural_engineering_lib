...
| ETABS-W3-PROJECT-CRITERIA-RECOVERY | Audit current COM/session/optimizer owners and legacy VBA beam/column workflows; define the smallest PID-safe adapter, common candidate evaluator, ETABS design snapshot and owned-copy W3K/L transaction | Main Agent / sole Windows writer | accepted #940; static source/help audit only; separate from LIB-PRO-015 beam capability review | P1 | AUDIT DOCUMENTED — `docs/planning/etabs-w3-com-vba-and-reanalysis-plan.md` records P0/P1/P2 functions, acceptance gates and effort ranges. No ETABS, COM, Excel, export, analysis/design or model operation occurred. Implementation begins with PID targeting/state guard; installed actions remain separately gated. |
