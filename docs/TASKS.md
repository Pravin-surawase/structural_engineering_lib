...
|----|------|-------|-----|----------|--------|
| ETABS-EXCEL-BEAM-W2-BASELINE | Freeze and implement the complete read-only beam/model topology, result-provenance, and independent-frame-analysis feasibility contract before any optimization or write-back | Main Agent | one local contract packet plus separate Windows acceptance | P1 | 🟡 W2A MERGED / PHASE A PUSHED / W2B PUSHED / W2C FAIL-CLOSED HOLD — PR #896 merged W2A as `0f5c918e...`; static audit head `4841ab2a...` proves all 19 COM operations and Phase B head `395f0618...` freezes the observer/REST/Excel surface. Installed preflight proved exact source/model/runtime/lock/units and all 15 cases finished; CSI's documented leading Base row required a narrow `GetStories` repair, then the approved combination proved present but inactive. No force row was read and no result-selection setter is permitted. Final blocked evidence/closeout remains; `HELD_NOT_SUPPORTED`, no-analysis/no-mutation, and professional-review holds retained |
