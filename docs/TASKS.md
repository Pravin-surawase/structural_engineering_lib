...
| ETABS-LIVE-G0-ROUTE-GATE | Disable live routes by default and enforce loopback, HTTP/WebSocket auth and operation classification before any COM construction/attach | Main Agent / sole Windows writer | accepted PR #943 plus bounded preflight | P0 | ⏸ START HOLD — repository/runtime/OpenAPI/client controls are ready, but the preflight observed ETABS already running. Close ETABS/Excel and reverify absence before creating the exact G0 task/branch; denied paths must prove zero COM side effect and `/status` remains nonattaching. |
