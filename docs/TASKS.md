...
| W3-BEAM-LONG-TERM-PLAN-AMENDMENT | Audit runtime, public API/data, evaluator/search, solver, acquisition and owned-copy risks; update every maintained plan owner | Main Agent / three read-only subagents, sole writer | accepted PR #942 | P0 | ✅ COMPLETE ON MERGE — plan/evidence/task/handoff candidate; no ETABS, COM, Excel, export, solver, analysis/design or model call. Next implementation is G0 only after merge. |
| ETABS-LIVE-G0-ROUTE-GATE | Disable live routes by default and enforce loopback, HTTP/WebSocket auth and operation classification before any COM construction/attach | Main Agent / sole Windows writer | accepted long-term amendment | P0 | 📋 PLANNED — denied paths must prove zero COM side effect; `/status` remains nonattaching; mutation separately disabled. |
