...
| MAINT-COMMIT-GATE-CONSOLIDATION | Measure and remove duplicate commit-time validation without weakening required safety evidence | Main Agent / maintenance lane | accepted offline-foundations milestone; no overlap with installed ETABS evidence | P1 | 📋 PLANNED — current repository has 34 hooks, 11 unconditional, and this cross-surface milestone activates about 28. Retain touched-file hygiene/format/type/security checks; consolidate overlapping unconditional governance checks behind one content-bound receipt; keep broad assurance in the PR/hosted lane; prove before/after coverage and wall time. |
