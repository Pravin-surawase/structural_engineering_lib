...
| ETABS-LIVE-G0-ROUTE-GATE | Disable live routes by default and enforce loopback, HTTP/WebSocket auth and operation classification before any COM construction/attach | Main Agent / sole Windows writer | accepted preflight PR #944 | P0 | 🟡 LOCAL CANDIDATE on `codex/etabs-live-g0-route-gate` from `3fc9ddcd` — default live paths absent; startup/peer/scope/WebSocket denials prove zero COM side effect; `/status` remains nonattaching; 87 Python and 55 React tests plus lint/build/OpenAPI/API-contract checks pass. One quick/hook/hosted cycle remains. The same PR records the seven-milestone W3 cadence. |
| LIB-BEAM-S0-CHECK-TRUTH | Remove hidden effective-depth assumptions and make REST/WebSocket supplied-beam V2 semantics consume supported fields or fail/hold explicitly | Main Agent / sole Windows writer | accepted G0 | P0 | 📋 PLANNED — field migration ledger, PASS/FAIL/HOLD/ERROR envelope, exact OpenAPI/Python/TS/React/WebSocket/version parity; ETABS/Excel closed. |
