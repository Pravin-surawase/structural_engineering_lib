# Reference

**Type:** Mixed
**Last Updated:** 2026-08-07
**Files:** 28

## Config Files

- [api-manifest.json](api-manifest.json)
- [clause-map.json](clause-map.json)

## Documentation Files

| File | Title | Description | Lines |
|------|-------|-------------|-------|
| [3d-json-contract.md](3d-json-contract.md) |  | > **Version:** 1.0.0 > **Purpose:** Defines the JSON schema  | 443 |
| [3d-visualization-performance.md](3d-visualization-performance.md) |  | This document provides performance benchmarks for the 3D bea | 172 |
| [README.md](README.md) | Reference Documentation | Comprehensive lookup documentation for APIs, formulas, contr | 80 |
| [agent-automation-pitfalls.md](agent-automation-pitfalls.md) |  | <!-- lint-ignore-git --> > ⚠️ **Note:** This document includ | 633 |
| [api-levels.md](api-levels.md) |  | structural_lib exposes three API levels. Pick the one that m | 99 |
| [api-stability.md](api-stability.md) |  | > This document defines which parts of the library are safe  | 616 |
| [api.md](api.md) |  | Machine-readable signatures live in docs/reference/api-manif | 3916 |
| [automation-catalog.md](automation-catalog.md) |  | > **Purpose:** Complete reference of all automation scripts  | 2847 |
| [bbs-dxf-contract.md](bbs-dxf-contract.md) |  | This document defines the stable contracts for Bar Bending S | 116 |
| [clause-map.md](clause-map.md) | IS 456 Clause-to-Function Mapping | Maps IS 456:2000 (and IS 13920:2016) clauses to their implem | 189 |
| [deferred-integrations.md](deferred-integrations.md) |  | - Uses conservative default (0.7) in comparison scoring - Fu | 150 |
| [deprecation-policy.md](deprecation-policy.md) |  | This document defines the deprecation policy for the structu | 368 |
| [dxf-layer-standards.md](dxf-layer-standards.md) |  | This document defines CAD layer standards used in DXF drawin | 469 |
| [error-codes.md](error-codes.md) |  | > Auto-generated from Python/structural_lib/core/errors.py.  | 87 |
| [error-schema.md](error-schema.md) |  | *Structured error format for machine-readable and human-frie | 269 |
| [fastapi-rest-api.md](fastapi-rest-api.md) |  | The Structural Engineering Library provides a REST API via F | 585 |
| [insights-api.md](insights-api.md) |  | > **Status:** Preview (v0.13.0+) > **Stability:** Experiment | 994 |
| [is456-formulas.md](is456-formulas.md) |  | $$d = D - c_{clear} - \phi_{stirrup} - \frac{\phi_{main}}{2} | 243 |
| [known-pitfalls.md](known-pitfalls.md) |  | Use this as a checklist to avoid common mistakes when implem | 135 |
| [library-contract.md](library-contract.md) |  | This document defines the stability promises for the beam de | 102 |
| [repo-health-baseline-2026-01-07.md](repo-health-baseline-2026-01-07.md) |  | - This baseline is a point-in-time snapshot for trend tracki | 63 |
| [sdk-api-contract-v1.md](sdk-api-contract-v1.md) |  | This document defines the public API contract for the Struct | 928 |
| [tech-stack-rationale.md](tech-stack-rationale.md) |  | > Moved from agent-bootstrap.md to keep bootstrap lean. This | 61 |
| [third-party-licenses.md](third-party-licenses.md) |  | This repository is primarily standard-library based. Optiona | 51 |
| [troubleshooting.md](troubleshooting.md) |  | Use this when reporting bugs or debugging agent failures: ba | 322 |

## Subfolders

| Folder | Files | Description |
|--------|-------|-------------|
| [vendor/](vendor/) | 1762 |  |
