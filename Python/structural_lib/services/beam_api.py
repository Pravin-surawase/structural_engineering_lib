...
"""
Module:       beam_api
Description:  Beam design API functions (IS 456:2000).
...
def suggest_beam_design_improvements(
    *,
    ...
    suggestion_report = design_suggestions.suggest_improvements(
        design=design,
        span_mm=span_mm,
        mu_knm=mu_knm,
        vu_kn=vu_kn,
    ...
    suggestions = [
        ...
        for sug in suggestion_report.suggestions
    ]

    return DesignSuggestionsResult(
        suggestions=suggestions,
        total_count=suggestion_report.suggestions_count,
        high_impact_count=suggestion_report.high_impact_count,
        medium_impact_count=suggestion_report.medium_impact_count,
        low_impact_count=suggestion_report.low_impact_count,
        analysis_time_ms=suggestion_report.analysis_time_ms,
        engine_version=suggestion_report.engine_version,
    )
...
def design_from_input(
    ...
    *,
    ...
    if beam.has_multiple_cases:
        ...
        compliance_report = check_beam_is456(
            units=beam.units,
            cases=cases,
            b_mm=geom.b_mm,
            D_mm=geom.D_mm,
        ...
        governing = compliance_report.governing_case_id
        governing_case = next(
            (c for c in compliance_report.cases if c.case_id == governing),
            compliance_report.cases[0] if compliance_report.cases else None,
        )
