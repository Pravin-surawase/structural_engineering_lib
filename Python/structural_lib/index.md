# Structural Lib

> **Purpose:** IS 456:2000 RC beam design calculations

**Type:** Python Package
**Last Updated:** 2026-08-21
**Files:** 52

## Public API

- `__version__`
- `adapters`
- `api`
- `audit`
- `batch`
- `compliance`
- `costing`
- `detailing`
- `dxf_export`
- `etabs_import`
- `flexure`
- `imports`
- `inputs`
- `models`
- `rebar`
- `reports`
- `result_base`
- `serialization`
- `serviceability`
- `shear`

## Documentation Files

| File | Title | Description | Lines |
|------|-------|-------------|-------|
| [README.md](README.md) | structural_lib - Core Python Library | > **Purpose:** IS 456:2000 RC beam design calculations > **P | 100 |

## Python Files

| File | Description | Classes | Functions | Lines |
|------|-------------|---------|-----------|-------|
| [__init__.py](__init__.py) | Package:      structural_lib | 0 | 0 | 471 |
| [__main__.py](__main__.py) | Unified CLI entrypoint for structural_lib. | 0 | 15 | 1781 |
| [adapters.py](adapters.py) | Backward compatibility stub. | 0 | 0 | 17 |
| [api.py](api.py) | Backward compatibility stub. | 0 | 0 | 11 |
| [api_results.py](api_results.py) | Backward compatibility stub. | 0 | 0 | 18 |
| [audit.py](audit.py) | Backward compatibility stub. | 0 | 0 | 17 |
| [batch.py](batch.py) | Backward compatibility stub. | 0 | 0 | 13 |
| [bbs.py](bbs.py) | Backward compatibility stub. | 0 | 0 | 51 |
| [beam_pipeline.py](beam_pipeline.py) | Backward compatibility stub. | 0 | 0 | 26 |
| [calculation_report.py](calculation_report.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 26 |
| [compliance.py](compliance.py) | Backward compatibility stub. | 0 | 0 | 31 |
| [constants.py](constants.py) | Backward compatibility stub. | 0 | 0 | 16 |
| [costing.py](costing.py) | Backward compatibility stub. | 0 | 0 | 23 |
| [dashboard.py](dashboard.py) | Backward compatibility stub. | 0 | 0 | 18 |
| [data_types.py](data_types.py) | Backward compatibility stub. | 0 | 0 | 41 |
| [detailing.py](detailing.py) | Backward compatibility stub. | 0 | 0 | 21 |
| [ductile.py](ductile.py) | Backward compatibility stub. | 0 | 0 | 35 |
| [dxf_export.py](dxf_export.py) | Backward compatibility stub. | 0 | 0 | 54 |
| [error_messages.py](error_messages.py) | Backward compatibility stub. | 0 | 0 | 26 |
| [errors.py](errors.py) | Backward compatibility stub. | 0 | 0 | 52 |
| [etabs_import.py](etabs_import.py) | Backward compatibility stub. | 0 | 0 | 26 |
| [excel_bridge.py](excel_bridge.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 28 |
| [excel_integration.py](excel_integration.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 32 |
| [flexure.py](flexure.py) | Backward compatibility stub. | 0 | 0 | 25 |
| [imports.py](imports.py) | Backward compatibility stub. | 0 | 0 | 31 |
| [inputs.py](inputs.py) | Backward compatibility stub. | 0 | 0 | 20 |
| [intelligence.py](intelligence.py) | Backward compatibility stub. | 0 | 0 | 21 |
| [job_cli.py](job_cli.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 27 |
| [job_runner.py](job_runner.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 25 |
| [materials.py](materials.py) | Backward-compatibility stub — canonical location: codes/is45 | 0 | 0 | 11 |
| [models.py](models.py) | Backward compatibility stub. | 0 | 0 | 22 |
| [multi_objective_optimizer.py](multi_objective_optimizer.py) | Backward compatibility stub. | 0 | 0 | 16 |
| [optimization.py](optimization.py) | Backward compatibility stub. | 0 | 0 | 14 |
| [rebar.py](rebar.py) | Backward compatibility stub. | 0 | 0 | 13 |
| [rebar_optimizer.py](rebar_optimizer.py) | Backward compatibility stub. | 0 | 0 | 14 |
| [release_uat.py](release_uat.py) | CLI compatibility facade for the source-free exact-wheel UAT | 0 | 0 | 10 |
| [report.py](report.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 39 |
| [report_svg.py](report_svg.py) | Backward compatibility stub. *(stub)* | 0 | 0 | 23 |
| [result_base.py](result_base.py) | Backward compatibility stub. | 0 | 0 | 14 |
| [serialization.py](serialization.py) | Backward compatibility stub. | 0 | 0 | 21 |
| [serviceability.py](serviceability.py) | Backward compatibility stub. | 0 | 0 | 37 |
| [shear.py](shear.py) | Backward compatibility stub. | 0 | 0 | 21 |
| [slenderness.py](slenderness.py) | Backward compatibility stub. | 0 | 0 | 21 |
| [tables.py](tables.py) | Backward compatibility stub. | 0 | 0 | 31 |
| [testing_strategies.py](testing_strategies.py) | Backward compatibility stub. | 0 | 0 | 28 |
| [torsion.py](torsion.py) | Backward compatibility stub. | 0 | 0 | 23 |
| [traceability.py](traceability.py) | Backward compatibility stub. | 0 | 0 | 15 |
| [types.py](types.py) | Backward compatibility stub. | 0 | 0 | 28 |
| [utilities.py](utilities.py) | Backward compatibility stub. | 0 | 0 | 17 |
| [validation.py](validation.py) | Backward compatibility stub. | 0 | 0 | 24 |

## Subfolders

| Folder | Files | Description |
|--------|-------|-------------|
| [cli/](cli/) 📦 | 2 |  |
| [codes/](codes/) 📦 | 114 |  |
| [core/](core/) 📦 | 26 |  |
| [data/](data/) | 4 |  |
| [insights/](insights/) 📦 | 12 |  |
| [reports/](reports/) 📦 | 7 |  |
| [research/](research/) 📦 | 4 |  |
| [services/](services/) 📦 | 56 |  |
| [visualization/](visualization/) 📦 | 4 |  |
