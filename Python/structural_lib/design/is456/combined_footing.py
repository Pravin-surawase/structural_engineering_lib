...
from structural_lib.services.contracts.family_f3 import (
    CombinedActionsV1,
    CombinedEvidenceReviewV1,
    CombinedFootingInputV1,
    CombinedGeometryTopologyV1,
    CombinedIdentitySourceV1,
    CombinedMaterialsReinforcementV1,
)
...
def input(  # noqa: A001
    *,
    identity_source: CombinedIdentitySourceV1 | Mapping[str, object],
    geometry_topology: CombinedGeometryTopologyV1 | Mapping[str, object],
    actions: CombinedActionsV1 | Mapping[str, object],
    materials_reinforcement: CombinedMaterialsReinforcementV1 | Mapping[str, object],
    evidence_review: CombinedEvidenceReviewV1 | Mapping[str, object],
) -> CombinedFootingInputV1:
    """Build a validated combined footing request from explicit groups.

    Parameters
    ----------
    identity_source : CombinedIdentitySourceV1 or Mapping[str, object]
        Member/case identity and source references for the design basis. Mapping inputs use the same strict validation.
    geometry_topology : CombinedGeometryTopologyV1 or Mapping[str, object]
        Dimensions in mm and the supported arrangement and restraints. Mapping inputs use the same strict validation.
    actions : CombinedActionsV1 or Mapping[str, object]
        Factored design actions in the units named by each field. Mapping inputs use the same strict validation.
    materials_reinforcement : CombinedMaterialsReinforcementV1 or Mapping[str, object]
        Material strengths and explicit supplied/permitted reinforcement. Mapping inputs use the same strict validation.
    evidence_review : CombinedEvidenceReviewV1 or Mapping[str, object]
    ...
    Limitations
    -----------
    Column positions, soil basis and reinforcement evidence must match the supported symmetric case.
    Existing mapping-based callers remain supported; no engineering defaults are inferred.

    Examples
    --------
    See the executable combined-footing recipe in the family facade cookbook:
...
def load(
    value: Mapping[str, object] | CombinedFootingInputV1,
) -> CombinedFootingInputV1:
    """Validate a Python mapping or an existing CombinedFootingInputV1.

    Parameters
    ----------
    value : Mapping[str, object] or CombinedFootingInputV1
...
def design(
    request: CombinedFootingInputV1,
) -> CanonicalFamilyResultV1[SymmetricCombinedFootingDesignResult]:
    """Design a symmetric two-column combined footing.
