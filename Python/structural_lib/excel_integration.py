...
class BeamDesignData:
    ...
    def from_dict(cls, data: Dict) -> 'BeamDesignData':
        ...
        normalized = {}
        key_map = {
            'beamid': 'beam_id', 'beam_id': 'beam_id', 'BeamID': 'beam_id',
            'story': 'story', 'Story': 'story',
            'b': 'b', 'width': 'b',
            'd': 'D', 'D': 'D', 'depth': 'D',
            'eff_d': 'd', 'effective_depth': 'd',
            'span': 'span', 'Span': 'span',
            'cover': 'cover', 'Cover': 'cover',
            'fck': 'fck', 'Fck': 'fck',
            'fy': 'fy', 'Fy': 'fy',
            'mu': 'Mu', 'Mu': 'Mu', 'moment': 'Mu',
            'vu': 'Vu', 'Vu': 'Vu', 'shear': 'Vu',
            'ast_req': 'Ast_req', 'Ast_req': 'Ast_req', 'ast': 'Ast_req',
            'asc_req': 'Asc_req', 'Asc_req': 'Asc_req', 'asc': 'Asc_req',
            'stirrup_dia': 'stirrup_dia', 'Stirrup_Dia': 'stirrup_dia',
            'stirrup_spacing': 'stirrup_spacing', 'Stirrup_Spacing': 'stirrup_spacing',
...
def main():
    ...
    parser.add_argument(
        "input_file",
        help="CSV or JSON file with beam design data"
    )
    parser.add_argument(
        "-o", "--output",
        default="./dxf_output",
    ...
    parser.add_argument(
        "--seismic",
        action="store_true",
        help="Apply IS 13920 seismic detailing rules"
    )
    parser.add_argument(
        "--schedule",
        help="Export detailing schedule to CSV file"
    )
