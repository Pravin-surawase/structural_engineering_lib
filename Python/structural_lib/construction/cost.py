...
def _error(code: str, message: str, field: str, remediation: str) -> Diagnostic:
    return Diagnostic(code, "error", message, ESTIMATE_COST_OPERATION, field, "construction-cost", remediation)


def _reject(inputs: dict[str, dict[str, object]], code: str, message: str, field: str, remediation: str) -> OperationResult:
    return rejected_result(ESTIMATE_COST_OPERATION, inputs, (_error(code, message, field, remediation),), provenance=_provenance())
...
def estimate_construction_cost(request: ConstructionCostRequest) -> OperationResult:
    ...
    if not all(
        ...
        return _reject(inputs, "COST.IDENTITY", "Costing requires complete currency, date, geography, source, profile, and result identities.", "request", "Supply a complete dated rate profile.")
    try:
        date.fromisoformat(profile.valuation_date)
    except ValueError:
        return _reject(inputs, "COST.DATE", "Valuation date must be an ISO calendar date.", "rate_profile.valuation_date", "Supply YYYY-MM-DD.")
    if (
        ...
        return _reject(inputs, "COST.QUANTITY_STALE", "Cost must bind current quantities for the same project, member, and detail.", "quantities", "Recalculate quantities for the active detail.")
    if request.quantity_output_payload_id != semantic_hash(
        "output_payload_id", quantities
    ):
        return _reject(inputs, "COST.QUANTITY_BINDING", "The quantity payload does not match its canonical output identity.", "quantity_output_payload_id", "Bind the unchanged AO04 output payload.")
    ...
    if (
        ...
        return _reject(inputs, "COST.SCOPE", "Every direct-cost category must be explicitly included or excluded exactly once.", "rate_profile.scope", "Declare material, formwork, coupler, labour, and plant scope.")
    if not profile.rates or any(category not in {item.category for item in profile.rates} for category in included):
        return _reject(inputs, "COST.RATE_MISSING", "Every included cost category requires at least one explicit rate.", "rate_profile.rates", "Add the missing in-scope rate or explicitly exclude its category.")
    ...
    if len(rate_ids) != len(set(rate_ids)) or len(rate_keys) != len(set(rate_keys)):
        return _reject(inputs, "COST.RATE_DUPLICATE", "Rate identities and category/basis pairs must be unique.", "rate_profile.rates", "Remove overlapping rates that could price one scope twice.")
    if any(
        not all(_text(value) for value in (item.rate_id, item.description, item.source_reference))
        ...
        return _reject(inputs, "COST.RATE_INVALID", "Rates require an included category, supported basis, nonnegative decimal rate, description, and source.", "rate_profile.rates", "Correct the priced line scope and decimal rate.")
    ...
    if any(rate.basis not in permitted_bases[rate.category] for rate in profile.rates):
        return _reject(inputs, "COST.RATE_SCOPE", "A cost category may price only its declared physical quantity basis.", "rate_profile.rates", "Bind material, formwork, and coupler rates to their matching quantities.")
    ...
    if profile.waste_pricing_basis is WastePricingBasis.SCHEDULED_STEEL:
        ...
        return _reject(inputs, "COST.WASTE_POLICY", "A single supported steel waste pricing basis is required.", "rate_profile.waste_pricing_basis", "Choose scheduled steel or purchased stock.")
    if any(item.basis is forbidden for item in profile.rates):
        return _reject(inputs, "COST.WASTE_DOUBLE_COUNT", "The selected steel waste basis conflicts with a supplied steel rate basis.", "rate_profile.rates", "Price steel from exactly one scheduled-or-purchased basis.")
    required_bases: dict[CostCategory, set[CostBasis]] = {
        CostCategory.MATERIAL: {
            (
                CostBasis.STEEL_SCHEDULED_MASS_KG
                if profile.waste_pricing_basis
                is WastePricingBasis.SCHEDULED_STEEL
    ...
    if any(
        ...
        return _reject(inputs, "COST.RATE_COVERAGE", "The included material, formwork, or coupler scope is missing a required physical quantity rate.", "rate_profile.rates", "Price every physical basis in the declared included scope.")
    ...
    if overhead_percent is None or tax_percent is None:
        return _reject(inputs, "COST.PERCENT", "Overhead and tax treatments require explicit nonnegative decimal percentages.", "rate_profile", "Supply zero when the declared scope excludes an amount.")
    basis_values = {
    ...
    overhead = (subtotal * overhead_percent / Decimal(100)).quantize(
        Decimal("0.01")
    )
    ...
    return completed_result(ESTIMATE_COST_OPERATION, inputs, {"cost": output}, provenance=_provenance())
