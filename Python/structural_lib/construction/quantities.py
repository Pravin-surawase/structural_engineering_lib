...
def calculate_construction_quantities(
    ...
    if not all(
        ...
        return _reject(inputs, "QUANTITY.IDENTITY", "Complete quantity, detail, and measurement policy identities are required.", "request", "Supply all current identities and named policies.")
    if (
        ...
        return _reject(inputs, "QUANTITY.BBS_STALE", "Quantities must bind the current passing BBS for the same detail.", "bbs", "Recreate the BBS from the active resolved schedule.")
    if request.bbs_output_payload_id != semantic_hash("output_payload_id", bbs):
        return _reject(inputs, "QUANTITY.BBS_BINDING", "The BBS payload does not match its canonical output identity.", "bbs_output_payload_id", "Bind the unchanged AO19 output payload.")
    ...
    deduction_ids = [item.deduction_id for segment in request.concrete_segments for item in segment.deductions]
    deduction_owners = [item.ownership_id for segment in request.concrete_segments for item in segment.deductions]
    if (
        ...
        or any(
            not all(_text(value) for value in (item.segment_id, item.member_id, item.material_id, item.ownership_id))
            ...
            or any(
                not all(_text(value) for value in (deduction.deduction_id, deduction.ownership_id, deduction.reason))
        ...
        return _reject(inputs, "QUANTITY.CONCRETE_OWNERSHIP", "Concrete segments and deductions require unique, explicit physical ownership.", "concrete_segments", "Remove duplicate or unbound segment and deduction ownership.")
    ...
    for segment in request.concrete_segments:
        ...
        if deductions > gross + 1e-12:
            return _reject(inputs, "QUANTITY.CONCRETE_DEDUCTION", "Concrete deductions cannot exceed the owned gross segment volume.", f"concrete_segments[{segment.segment_id}]", "Correct the explicit overlap or opening deduction.")
    ...
    area_deduction_ids = [item.deduction_id for face in request.formwork_faces for item in face.deductions]
    area_deduction_owners = [item.ownership_id for face in request.formwork_faces for item in face.deductions]
    if (
        ...
        or any(
            not all(_text(value) for value in (item.face_id, item.member_id, item.ownership_id))
            or item.member_id != request.member_id
            ...
            or (item.measurement_state is FormworkMeasurementState.EXCLUDED and not _text(item.exclusion_reason))
            or (item.measurement_state is FormworkMeasurementState.INCLUDED and item.exclusion_reason is not None)
            or any(
                not all(_text(value) for value in (deduction.deduction_id, deduction.ownership_id, deduction.reason))
        ...
        return _reject(inputs, "QUANTITY.FORMWORK_OWNERSHIP", "Formwork faces and deductions require unique physical ownership and explicit inclusion state.", "formwork_faces", "Itemize each contact or interface face exactly once.")
    ...
    for face in request.formwork_faces:
        ...
        if deductions > gross + 1e-12:
            return _reject(inputs, "QUANTITY.FORMWORK_DEDUCTION", "Formwork deductions cannot exceed the owned gross contact face.", f"formwork_faces[{face.face_id}]", "Correct the explicit contact-area deduction.")
        net = gross - deductions if face.measurement_state is FormworkMeasurementState.INCLUDED else 0.0
    ...
    output = ConstructionQuantityOutput(
        ...
        WasteLedger(bbs.kerf_length_mm, bbs.reusable_offcut_length_mm, bbs.waste_length_mm),
