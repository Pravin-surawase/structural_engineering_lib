# Is456

**Type:** Python Package
**Last Updated:** 2026-08-22
**Files:** 15

## Public API

- `IS456Code`
- `beam`
- `column`
- `footing`
- `slab`
- `tables`
- `shear`
- `flexure`
- `detailing`
- `serviceability`
- `compliance`
- `slenderness`
- `torsion`
- `traceability`
- `clause`
- `get_clause_refs`
- `get_clause_info`
- `list_clauses_by_category`
- `search_clauses`

## Config Files

- [clauses.json](clauses.json)

## Python Files

| File | Description | Classes | Functions | Lines |
|------|-------------|---------|-----------|-------|
| [__init__.py](__init__.py) | IS 456:2000 - Indian Standard for Plain and Reinforced Concr | 1 | 0 | 151 |
| [_validation.py](_validation.py) | Private scalar validation shared by lower-level IS 456 helpe | 0 | 2 | 41 |
| [compliance.py](compliance.py) | Module: compliance | 0 | 3 | 640 |
| [detailing.py](detailing.py) | Backward compatibility shim — detailing module moved to beam | 0 | 0 | 28 |
| [ductile.py](ductile.py) | Backward compatibility shim — ductile module moved to codes/ | 0 | 0 | 30 |
| [flexure.py](flexure.py) | Backward compatibility shim — flexure module moved to beam/  | 0 | 0 | 28 |
| [load_analysis.py](load_analysis.py) | Module:       load_analysis | 0 | 7 | 844 |
| [materials.py](materials.py) | Module:       materials | 0 | 4 | 123 |
| [serviceability.py](serviceability.py) | Backward compatibility shim — serviceability module moved to | 0 | 0 | 37 |
| [shear.py](shear.py) | Backward compatibility shim — shear module moved to beam/ su | 0 | 0 | 28 |
| [slenderness.py](slenderness.py) | Slenderness Check Module — IS 456:2000 Beam Lateral Stabilit | 2 | 3 | 364 |
| [tables.py](tables.py) | Module:       tables | 0 | 2 | 121 |
| [torsion.py](torsion.py) | Backward compatibility shim — torsion module moved to beam/  | 0 | 0 | 29 |
| [traceability.py](traceability.py) | IS 456 Traceability Module | 0 | 9 | 350 |

## Subfolders

| Folder | Files | Description |
|--------|-------|-------------|
| [beam/](beam/) 📦 | 6 |  |
| [column/](column/) 📦 | 12 |  |
| [combined_footing/](combined_footing/) 📦 | 6 |  |
| [common/](common/) 📦 | 5 |  |
| [deep_beam/](deep_beam/) 📦 | 6 |  |
| [flat_slab/](flat_slab/) 📦 | 8 |  |
| [footing/](footing/) 📦 | 10 |  |
| [slab/](slab/) 📦 | 16 |  |
| [staircase/](staircase/) 📦 | 7 |  |
| [strap_footing/](strap_footing/) 📦 | 6 |  |
| [wall/](wall/) 📦 | 6 |  |
