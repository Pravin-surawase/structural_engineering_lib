# Column

**Type:** Python Package
**Last Updated:** 2026-08-10
**Files:** 10

## Public API

- `axial`
- `biaxial`
- `biaxial_bending_check`
- `calculate_additional_moment`
- `check_helical_reinforcement`
- `create_column_detailing`
- `create_symmetric_two_face_layout`
- `design_long_column`
- `design_short_column_uniaxial`
- `detailing`
- `helical`
- `long_column`
- `pm_interaction_curve`
- `pm_interaction_slice_for_layout`
- `pmm`
- `experimental_pmm_interaction_surface`
- `slenderness`
- `uniaxial`

## Python Files

| File | Description | Classes | Functions | Lines |
|------|-------------|---------|-----------|-------|
| [__init__.py](__init__.py) | IS 456:2000 — Column design modules. | 0 | 0 | 63 |
| [_common.py](_common.py) | Module:       _common | 0 | 0 | 68 |
| [axial.py](axial.py) | Module:       axial | 0 | 4 | 358 |
| [biaxial.py](biaxial.py) | Module:       biaxial | 0 | 1 | 517 |
| [detailing.py](detailing.py) | Module:       detailing | 0 | 8 | 634 |
| [helical.py](helical.py) | Module:       helical | 0 | 1 | 237 |
| [long_column.py](long_column.py) | Module:       long_column | 0 | 1 | 416 |
| [pmm.py](pmm.py) | Experimental strain-compatibility analysis for rectangular c | 0 | 3 | 510 |
| [slenderness.py](slenderness.py) | Module:       slenderness | 0 | 1 | 225 |
| [uniaxial.py](uniaxial.py) | Module:       uniaxial | 0 | 2 | 880 |
