...
def print_schedule(results: List[BeamDesignOutput]):
    ...
    # Header
    print(f"{'Beam':<6} {'Story':<8} {'Size':<10} {'Span':<6} "
    ...
    for r in results:
        ...
        print(f"{r.beam_id:<6} {r.story:<8} {r.size:<10} {int(r.span):<6} "
...
def save_schedule_csv(results: List[BeamDesignOutput], filepath: str):
    ...
    fieldnames = [
        'BeamID', 'Story', 'Size', 'Span', 'Mu_lim', 'Ast_req', 'Asc_req',
        'Section_Type', 'Bottom_Bars', 'Top_Bars', 'Ast_prov', 'Asc_prov',
        'Vu', 'Tv', 'Tc', 'Stirrup_Spacing', 'Stirrups',
        'Ld', 'Lap_Length', 'Bar_Spacing', 'Status', 'Remarks'
    ]
...
def main():
    ...
    for beam in beams:
        try:
            ...
            print(f"  {status} {result.beam_id}: {result.bottom_bars}, {result.stirrups}")
