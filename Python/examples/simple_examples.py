...
def example_7_complete_design():
    ...
    print(f"Bar spacing = {spacing:.0f} mm (min = {detailing.get_min_spacing(main_bar):.0f} mm)")
    
    print("\n--- SUMMARY ---")
    print(f"Beam {beam_id}: {b}×{D} mm")
    print(f"Bottom: {n_bars}-{main_bar}φ | Top: 2-12φ | Stirrups: 2L-{stirrup_dia}φ@{int(sv)}")
