...
def test_is456_golden_vectors(vector: dict):
    ...
    assert res.shear.tau_v == pytest.approx(exp["shear"]["tv"], rel=1e-12, abs=1e-12)
    assert res.shear.tau_c == pytest.approx(exp["shear"]["tc"], rel=1e-12, abs=1e-12)
    assert res.shear.tau_c_max == pytest.approx(
