...
export function beamRowInputs(beam: BeamCSVRow): { [key: string]: JsonValue } {
  return {
    widthMm: beam.b,
    depthMm: beam.D,
    effectiveDepthMm: beam.d_mm ?? null,
    stirrupDiameterMm: beam.stirrup_diameter_mm ?? null,
    tensionBarDiameterMm: beam.tension_bar_diameter_mm ?? null,
    spanMm: beam.span,
    fckMpa: beam.fck ?? 25,
    fyMpa: beam.fy ?? 500,
...
export function workspaceSnapshotToBeamRows(snapshot: WorkspaceSnapshotV1): BeamCSVRow[] {
  return snapshot.members.map((member) => {
    const beam: BeamCSVRow = {
      ...
      d_mm: optionalNumericInput(member, 'effectiveDepthMm'),
      stirrup_diameter_mm: optionalNumericInput(member, 'stirrupDiameterMm'),
      tension_bar_diameter_mm: optionalNumericInput(member, 'tensionBarDiameterMm'),
