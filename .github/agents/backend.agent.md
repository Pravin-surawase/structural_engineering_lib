...
| Module | Functions |
|--------|-----------|
| `services/api.py` | 68 public + 15 internal — `design_beam_is456()`, `detail_beam_is456()`, `optimize_beam_cost()` |
...
- **Use `time.perf_counter()` for all timing measurements, NEVER `time.time()`.** `time.time()` has ~15ms resolution on Windows and is affected by NTP clock adjustments. `time.perf_counter()` is monotonic and high-resolution on all platforms. This applies to performance logging, timeout detection, and any elapsed-time calculation. (Session 14: 6 files used `time.time()` inconsistently when `time.perf_counter()` was already the standard in other modules.)
