...
8. **Update OpenAPI baseline after model changes:** After ANY change to Pydantic model `Field(description=...)`, response model fields, or route metadata, run `./scripts/python_runtime.sh scripts/check_openapi_snapshot.py --update` and commit the updated `fastapi_app/openapi_baseline.json`. The same full-spec checker runs in PR and nightly validation.
