...
0. **After validation tightening, audit ALL existing tests** — New constraints (e.g., `d_mm < D_mm`) will reject previously-accepted test data. Search tests for the tightened parameter and fix any violations BEFORE committing. (Sprint 1 v0.21.5: `test_compare_three_designs` used `d_mm=500` with `D_mm=500`, caught by reviewer.)
...
- **Use `time.perf_counter()` for timing assertions, NEVER `time.time()`.** `time.time()` has ~15ms resolution on Windows, causing tests with tight timing tolerances (e.g., `< 5.0s`) to flake. Always use `time.perf_counter()` for elapsed-time measurements. When writing timing-sensitive tests, use generous tolerances (2x expected) to account for CI runners. (Session 14, PR #550: Windows CI timing failures were 1 of 5 undetected daily failures.)
