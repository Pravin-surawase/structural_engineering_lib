...
- Run only necessary targeted diagnostics while editing, then one consolidated
  focused selection after content freezes. Let the batched PR own comprehensive
  assurance. Follow `AGENTS.md` for explicit broad local gates: only at a named
  cumulative/release boundary or for repository-wide investigation.
