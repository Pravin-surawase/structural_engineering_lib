...
def compute_summary_stats(trials: List[Dict]) -> Dict:
    ...
    return {
        ...
        "error_rate": (
            statistics.mean(errors) / statistics.mean(files)
            if files and sum(files) > 0
            else 0
        ),
    }
...
def analyze_baseline():
    ...
    # Save summary
    output_file = Path(
        "docs/research/navigation_study/data/processed/baseline_summary.json"
    )
...
def analyze_hierarchical():
    ...
    # Save summary
    output_file = Path(
        "docs/research/navigation_study/data/processed/hierarchical_summary.json"
    )
...
def compare_conditions():
    ...
    for agent in ["gpt4_turbo", "claude_sonnet", "llama3_70b"]:
        if agent in baseline["agents"] and agent in hierarchical["agents"]:
            ...
            time_speedup = (
                b["time_to_complete"]["mean_ms"] / h["time_to_complete"]["mean_ms"]
                if h["time_to_complete"]["mean_ms"] > 0
                else 0
            )
            token_reduction = (
                (b["tokens_loaded"]["mean"] - h["tokens_loaded"]["mean"])
                / b["tokens_loaded"]["mean"]
                * 100
                if b["tokens_loaded"]["mean"] > 0
                else 0
            )
    ...
    # Save comparison
    output_file = Path(
        "docs/research/navigation_study/data/processed/comparison_stats.json"
    )
