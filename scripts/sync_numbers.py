...
def scan_api_functions() -> tuple[int, int]:
    """Count exported API functions and split-module private helpers."""
    ...
    private = 0
    for module_name in ("beam_api.py", "column_api.py", "common_api.py"):
        content = (python_root / "structural_lib" / "services" / module_name).read_text(
...
def get_update_rules() -> list[UpdateRule]:
    ...
    return [
        ...
        # README.md — public API function counts
        (
            "README.md",
            r"\*\*API functions\*\* \| (\d+) public functions",
            "**API functions** | {value} public functions",
            "api_public",
        ),
        (
            "README.md",
            r"library exposes (\d+) public functions",
            "library exposes {value} public functions",
            "api_public",
        ),
...
def find_updates(metrics: Metrics) -> list[Update]:
    ...
    for file_rel, pattern, template, metric_name in get_update_rules():
        ...
        for i, line in enumerate(lines):
            ...
            elif metric_name == "api_public":
                old_val = int(match.group(1))
                new_val = metrics.api_public_count
                if old_val == new_val:
                    continue
                new_text = (
                    line[: match.start()]
