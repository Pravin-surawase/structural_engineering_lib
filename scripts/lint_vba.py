...
class VBALinter:
    ...
    def check_option_explicit(self):
        ...
        if not re.search(
            r"^\s*Option\s+Explicit", self.content, re.MULTILINE | re.IGNORECASE
        ):
    ...
    def check_procedure_pairing(self):
        ...
        for i, line in enumerate(self.lines, start=1):
            ...
            # Sub/Function start
            if re.match(
                r"^(Public|Private|Friend)?\s*(Sub|Function)", line_clean, re.IGNORECASE
            ):
                proc_type = re.search(
                    r"(Sub|Function)", line_clean, re.IGNORECASE
                ).group(1)
            ...
            if re.match(r"^End\s+(Sub|Function)", line_clean, re.IGNORECASE):
                end_type = re.search(
                    r"End\s+(Sub|Function)", line_clean, re.IGNORECASE
                ).group(1)
                if not proc_stack:
                    self.errors.append(
                        (i, f"'End {end_type}' without matching '{end_type}'")
                    )
                else:
                    ...
                    if start_type.lower() != end_type.lower():
                        self.errors.append(
                            (
                                i,
                                f"'End {end_type}' does not match '{start_type}' at line {start_line}",
                            )
                        )
        ...
        for start_line, proc_type in proc_stack:
            self.errors.append(
                (start_line, f"'{proc_type}' not closed with 'End {proc_type}'")
            )
    ...
    def check_undeclared_variables(self):
        ...
        for line in self.lines:
            ...
            if re.search(r"\b(Dim|Const|Public|Private)\s+(\w+)", line, re.IGNORECASE):
                var_name = re.search(
                    r"\b(Dim|Const|Public|Private)\s+(\w+)", line, re.IGNORECASE
                ).group(2)
        ...
        for i, line in enumerate(self.lines, start=1):
            ...
            if line.strip().startswith("'") or re.search(
                r"\b(Dim|Const|Public|Private)\s+", line, re.IGNORECASE
            ):
                continue
            ...
            if match:
                ...
                if var_name not in declared and var_name not in {
                    "set",
                    "let",
                    "get",
                    "call",
                    ...
                    self.warnings.append(
                        (i, f"Possible undeclared variable: '{match.group(1)}'")
                    )
    ...
    def check_common_typos(self):
        ...
        for i, line in enumerate(self.lines, start=1):
            if re.search(r"\bElse\s+If\b", line, re.IGNORECASE):
                self.warnings.append(
                    (i, "Use 'ElseIf' (one word) instead of 'Else If'")
                )
            ...
            if re.search(r"\bEndFunction\b", line, re.IGNORECASE):
                self.errors.append(
                    (i, "Use 'End Function' (two words) instead of 'EndFunction'")
                )
