...
def test_sample_fields():
    b = sample["beams"][0]
    required = ["id", "story", "width_mm", "depth_mm", "span_mm", "mu_knm", "vu_kn", "fck_mpa", "fy_mpa", "cover_mm", "point1", "point2"]
...
def test_sample_response_shape():
    expected_keys = {"success", "message", "beam_count", "beams", "format_detected", "warnings"}
    actual_keys = set(sample.keys())
    assert expected_keys == actual_keys, f"Key diff: expected={expected_keys}, got={actual_keys}"
...
def test_single_csv():
    ...
    single_result = api_post_form(
        "/api/v1/import/csv",
        {"format": "auto"},
        {"file": ("beam_forces.csv", content)}
    )
    assert single_result["success"], f"Not successful: {single_result.get('message')}"
    assert single_result["beam_count"] == 153, f"Expected 153, got {single_result['beam_count']}"
...
def test_single_csv_shape():
    ...
    assert expected_keys == actual_keys, f"Key diff: expected={expected_keys}, got={actual_keys}"
test("Single CSV response has exact expected keys (no column_mapping)", test_single_csv_shape)
...
def test_dual_csv():
    ...
    assert dual_result["beam_count"] > 0, f"Got 0 beams"
test("POST /import/dual-csv returns beams", test_dual_csv)

def test_dual_csv_shape():
    expected_keys = {"success", "message", "beam_count", "beams", "format_detected", "warnings", "unmatched_beams", "unmatched_forces"}
    actual_keys = set(dual_result.keys())
    assert expected_keys == actual_keys, f"Key diff: expected={expected_keys}, got={actual_keys}"
test("Dual CSV response has exact expected keys", test_dual_csv_shape)

def test_dual_csv_3d():
    beams_with_3d = [b for b in dual_result["beams"] if b.get("point1") and b.get("point2")]
    assert len(beams_with_3d) == dual_result["beam_count"], f"Only {len(beams_with_3d)}/{dual_result['beam_count']} have 3D"
...
def test_batch_design():
    global batch_result
    payload = [
        {"id": "B1", "width_mm": 230, "depth_mm": 450, "span_mm": 5000,
         "mu_knm": 100, "vu_kn": 50, "fck_mpa": 25, "fy_mpa": 500, "cover_mm": 40},
        {"id": "B2", "width_mm": 300, "depth_mm": 500, "span_mm": 6000,
         "mu_knm": 200, "vu_kn": 80, "fck_mpa": 30, "fy_mpa": 500, "cover_mm": 40},
    ]
...
def test_batch_result_shape():
    ...
    expected = {"beam_id", "success", "ast_required", "asc_required", "stirrup_spacing", "is_safe", "utilization_ratio", "error"}
    actual = set(r.keys())
    assert expected == actual, f"Key diff: expected={expected}, got={actual}"
test("Batch result has correct flat shape (not nested DesignedBeam)", test_batch_result_shape)
