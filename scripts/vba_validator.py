    ...
    def _check_syntax_errors(self, lines: List[str]):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if double_quote_count % 2 != 0:
                self.errors.append((i, "Mismatched quotes (odd number of unescaped quotes)"))
        ...
        for i, line in enumerate(lines, 1):
            for bad_char, fix in unicode_patterns.items():
                if bad_char in line:
                    ...
                    self.errors.append((i, f"Unicode character {char_repr!r} found - replace with {fix!r}"))
            ...
            if re.search(r'\bCall\s+\w+\s+[^(]', code_part):
                ...
                if match:
                    ...
                    if func_name not in ['Exit', 'End', 'Next', 'Loop']:
                        self.errors.append((i, f"Call statement missing parentheses: Call {func_name}(...) required"))
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'As Integer' in line:
                if 'For ' in line or 'count' in line.lower() or 'size' in line.lower():
                    self.warnings.append((i, f"Consider using 'As Long' instead of 'As Integer' for loop/size variables"))
            
            # Check for division without CDbl
            if '/' in line and 'CDbl' not in line and "'" not in line[:line.find('/')]:
                if not any(x in line for x in ['http', 'file', '<!--']):  # Skip URLs/comments
                    pass  # Too many false positives
            ...
            if 'On Error Resume Next' in line:
                ...
                if i + 5 < len(lines):
                    ...
                    if 'On Error GoTo 0' not in next_5_lines:
                        self.warnings.append((i, "On Error Resume Next without On Error GoTo 0 - error may persist"))
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r'\.\w+\s*[=\[]', line):  # obj.property = or obj.method(
                ...
                if i > 1:
                    ...
                    if 'Is Nothing' not in prev_lines and 'Is Not Nothing' not in prev_lines:
                        # Only warn for known object types
                        if any(x in line for x in ['sapModel', 'etabs', 'ws', 'wb', 'comp']):
                            pass  # Too many false positives
    ...
    def _check_standards(self, lines: List[str]):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'C:\\' in line and "'" not in line[:line.find('C:')]:  # Not in comment
                self.warnings.append((i, "Hardcoded Windows path found - use Application.PathSeparator instead"))
            ...
            if 'Public Enum' in line:
                self.warnings.append((i, "Enum detected - consider using Constants instead to avoid ambiguity"))
