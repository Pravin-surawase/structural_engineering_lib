...
Usage:
    python scripts/check_streamlit.py --all-pages
    python scripts/check_streamlit.py --page beam_design
    python scripts/check_streamlit.py tmp/test_file.py
    python scripts/check_streamlit.py --all-pages --fail-on critical,high
    python scripts/check_streamlit.py --all-pages --ignore-file .scanner-ignore.yml
    python scripts/check_streamlit.py --all-pages --verbose
    python scripts/check_streamlit.py --fragments              # Fragment API violations only
    python scripts/check_streamlit.py --fragments --file path  # Check specific file
"""
...
# ─── Fragment API Violation Detection ────────────────────────────────────────
...
class FragmentViolationDetector(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        is_fragment = any(
            self._is_fragment_decorator(d) for d in node.decorator_list
        )
        if is_fragment:
            prev = self.current_fragment
            self.current_fragment = node.name
            self.generic_visit(node)
            self.current_fragment = prev
        else:
            self.generic_visit(node)
            ...
            for item in node.items:
                ...
                if (
                    ...
                    self.violations.append((
                        node.lineno,
                        self.current_fragment,
                        f"'with st.sidebar:' in fragment '{self.current_fragment}'",
                    ))
        self.generic_visit(node)

    @staticmethod
    def _is_fragment_decorator(decorator: ast.expr) -> bool:
...
def _check_fragments_file(filepath: Path) -> list[tuple[str, int, str, str]]:
    ...
    try:
        ...
        detector.visit(tree)
        return [(str(filepath), ln, frag, msg) for ln, frag, msg in detector.violations]
    except SyntaxError as e:
...
def _check_fragments_directory(directory: Path) -> list[tuple[str, int, str, str]]:
    ...
    for filepath in directory.rglob("*.py"):
        if "/tests/" in str(filepath) or "/__pycache__/" in str(filepath):
            continue
        all_violations.extend(_check_fragments_file(filepath))
...
def _run_fragment_check(args: argparse.Namespace) -> int:
    ...
    if hasattr(args, "file") and args.file:
        ...
        if not path.exists():
            ...
            return 2
        violations = _check_fragments_file(path)
    else:
        streamlit_dir = project_root / "streamlit_app"
        if not streamlit_dir.exists():
            print(f"❌ Directory not found: {streamlit_dir}", file=sys.stderr)
            return 2
        violations = _check_fragments_directory(streamlit_dir)
    ...
    for filepath, lineno, _frag, msg in violations:
        by_file[filepath].append((lineno, msg))

    for filepath, file_violations in sorted(by_file.items()):
        print(f"📄 {filepath}:")
        for lineno, msg in sorted(file_violations):
...
def main():
    ...
    parser.add_argument(
        "--fragments",
        action="store_true",
        help="Run fragment API violation detection (from check_fragment_violations.py)",
    )
    parser.add_argument(
        "--file",
        type=str,
        help="Check specific file (used with --fragments)",
    )
    ...
    # Fragment-only mode
    if args.fragments:
        return _run_fragment_check(args)
