            ...
            if "st.session_state." in line and "[" not in line:
                ...
                if match:
                    ...
                    if not has_check:
                        ...
                        check_line = (
                            " " * indent + f"if '{key}' not in st.session_state:"
                        )
                        init_line = (
                            " " * (indent + 4)
                            + f"st.session_state.{key} = None  # Initialize"
                        )
            ...
            if "/" in line and "//" not in line and "http" not in line.lower():
                ...
                if match:
                    ...
                    for j in range(max(0, i - 3), i):
                        if denominator in lines[j] and (
                            "if" in lines[j] or "!= 0" in lines[j] or "> 0" in lines[j]
                        ):
                            has_check = True
                            break

                    if not has_check and not any(
                        keyword in line for keyword in ["def ", "class ", "import "]
                    ):
                        ...
                        if "=" in line_content:
                            ...
                            if not ("if" in expression and "else" in expression):
                                line = (
                                    " " * indent
                                    + f"{assignment} = ({expression}) if {denominator} > 0 else 0"
                                )
                                self.fixes_applied.append(
                                    Fix(
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Report fixes without applying"
    )
    parser.add_argument(
