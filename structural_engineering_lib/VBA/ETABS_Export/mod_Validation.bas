...
Public Function GetAndValidateUnits(sapModel As Object) As UnitConversion
    ...
    Select Case lengthUnits
        ...
        Case 5  ' cm
            units.LengthUnit = "cm"
            units.LengthToMM = 10#
        Case 6  ' Could be micron OR an undocumented unit
            ' Log all details for diagnosis
            LogWarning "ETABS length unit code: 6 (not standard)"
            LogInfo "Force unit: " & forceUnits & ", Temp unit: " & tempUnits
            
            ' Make educated guess based on force unit
            If forceUnits = 4 Or forceUnits = 3 Then
                ' kN or N suggests metric - assume meters
                LogInfo "Assuming METERS (metric force units detected)"
                units.LengthUnit = "m"
                units.LengthToMM = 1000#
            ElseIf forceUnits = 1 Or forceUnits = 2 Then
                ' lb or kip suggests imperial - assume inches
                LogInfo "Assuming INCHES (imperial force units detected)"
                units.LengthUnit = "in"
                units.LengthToMM = 25.4
            Else
                ' Unknown combination - default to meters
                LogWarning "Cannot determine unit - defaulting to METERS"
                units.LengthUnit = "m_assumed"
                units.LengthToMM = 1000#
            End If
            
            LogWarning "IMPORTANT: Verify length dimensions in exported data!"
        Case Else
            LogError "Unknown length unit: " & lengthUnits
            LogError "ETABS may be using custom units or newer API version"
            LogInfo "Defaulting to meters (1000mm) - please verify results!"
            units.LengthUnit = "m_assumed"
            units.LengthToMM = 1000#
            ' Don't fail - continue with assumption
    End Select
