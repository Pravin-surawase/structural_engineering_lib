...
Public Function ExportFrameForces(sapModel As Object, outputFolder As String) As Boolean
    ...
    For Each tableName in tableNames
        ...
        LogDebug "  Parameters: csvPath=" & csvPath
        LogDebug "  GroupType=All(0), GroupName=(empty)"

        ' GetTableForDisplayCSVFile(TableName, FileName, Append, GroupType, GroupName)
        ...
        LogDebug "    API returned: ret=" & ret & ", err=" & Err.Number
        
        ' Verify file was actually created with data
        If Err.Number = 0 And ret = 0 Then
            If FileExists(csvPath) Then
                Dim tempRows As Long
                tempRows = CountCSVRows(csvPath)
                LogDebug "    File created with " & tempRows & " rows"
                
                If tempRows > 1 Then  ' Has header + at least 1 data row
                    LogInfo "? Export successful using table: " & tableName
                    LogInfo "  File created: " & csvPath
                    success = True
                    Exit For
                Else
                    LogDebug "    File has no data rows (analysis may not be run)"
                End If
            Else
                LogDebug "    File was not created (no results available)"
            End If
        Else
            LogDebug "  Failed with table '" & tableName & "'"
            LogDebug "    Return code: " & ret
            LogDebug "    VBA Error: " & Err.Number & " - " & Err.Description
        End If
...
Private Function ExportFrameForcesDirect(sapModel As Object, csvPath As String) As Boolean
    ...
    ' Select all load cases for output (required before getting results)
    On Error Resume Next
    Dim ret As Long
    ret = sapModel.Results.Setup.DeselectAllCasesAndCombosForOutput()
    LogDebug "Deselected all cases: ret=" & ret & ", err=" & Err.Number
    ...
    LogDebug "Got " & numCases & " load cases, ret=" & ret
    
    If numCases = 0 Then
        LogError "No load cases found in model"
        LogError "This means:"
        LogError "  1. Analysis has never been run, OR"
        LogError "  2. No load cases are defined in the model, OR"
        LogError "  3. Results have been cleared"
        LogError ""
        LogError "SOLUTION: In ETABS, press F5 to run analysis"
        LogError "  Then re-run this export macro"
    ...
    LogDebug "GetNameList returned: ret=" & ret & ", count=" & frameCount & ", err=" & Err.Number

    If ret <> 0 Or frameCount = 0 Then
        LogError "Cannot get frame list: ret=" & ret & ", count=" & frameCount
        LogError "  VBA Error: " & Err.Number & " - " & Err.Description
        ExportFrameForcesDirect = False
        Exit Function
    End If
