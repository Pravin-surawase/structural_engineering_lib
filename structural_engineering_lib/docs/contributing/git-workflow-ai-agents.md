...
### 2) Direct Commit (When should_use_pr.sh says OK)
```bash
./scripts/ai_commit.sh "docs: update guide"
```
Run `./scripts/should_use_pr.sh --explain` first. It determines the threshold based on file type and size.
...
A: Run `./scripts/should_use_pr.sh --explain` to check. Generally: small docs/scripts/tests (<50 lines) go direct; production code always needs PR.
