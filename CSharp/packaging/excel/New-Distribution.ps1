...
$manifest = [ordered]@{
    ...
    force_profile = 'wp10-group-static-frame-capture/v1; complete source group forces, exact assignment tables, source geometry and per-object matrices; required beams filtered locally; no model mutations'
    reference_force_profile = 'wp10-shared-static-frame-capture/v1; retained direct-getter comparison path'
    force_worker_request = 'structural.etabs_force_worker_request/v2'
    snapshot_transport = 'structural.analysis_snapshot_rows/v1; canonical v1 identity; 64 MiB encoded and 256 MiB expanded bounds; single-member CRC32/ISIZE validation; legacy JSON/gzip readers retained'
