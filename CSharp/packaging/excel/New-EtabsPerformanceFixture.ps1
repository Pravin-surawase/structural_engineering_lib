...
[CmdletBinding()]
param(
    [Parameter(Mandatory)][ValidateSet('small','medium','baseline')][string]$Size,
...
$baselineDesign = $Size -eq 'baseline'
$memberCount = if ($baselineDesign) { 3 } elseif ($Size -eq 'small') { 100 } else { 1000 }
$stationCount = if ($baselineDesign) { 17 } else { 100 }
$caseLoads = if ($baselineDesign) { [ordered]@{WP11_ULS=12.0;WP11_SLS=8.0;WP11_SUSTAINED=5.0} } else { [ordered]@{PF9_STATIC=8.0} }
$workload = if ($baselineDesign) { 'WP11-BASELINE' } else { "BENCH-ETABS-$($Size.ToUpperInvariant())" }
$expectedRows = $memberCount * $stationCount * $caseLoads.Count
New-Item -ItemType Directory -Path (Join-Path $destination 'model') -Force | Out-Null
$modelPath = Join-Path $destination "model\$workload.EDB"
...
$receipt = [ordered]@{
    ...
    workload=$workload; started_utc=[DateTimeOffset]::UtcNow.ToString('o')
    ...
    units='kN_m_C'; output_cases=@($caseLoads.Keys); member_ids=@(); observations=@{}; cleanup=@{}; failure=$null
}
if ($baselineDesign) {
    $receipt.schema_version='wp11-owned-baseline-fixture/v1'
    $receipt.fixture_basis='Three independently loaded ordinary simply supported rectangular beams; zero axial/minor/torsion demand; 17 genuine stations in independently analysed ULS, total-service and sustained-service cases. No copied/scaled force rows.'
    $receipt.design_basis=[ordered]@{concrete_strength_n_per_mm2=25.0;steel_yield_n_per_mm2=500.0;link_yield_n_per_mm2=415.0;steel_modulus_n_per_mm2=200000.0;nominal_cover_mm=35.0;aggregate_mm=20.0;support_width_mm=1000.0;seismic_basis='ordinary_is456';fire_basis='Owned nonbuilding calculation fixture: no specified fire-resistance rating';lateral_restraint_basis='Both support centres: global Y translation and X rotation restrained';design_basis_origin='owned_fixture_engineering_definition';production_approval=$false;self_weight_multiplier=0.0;case_udl_kn_per_m=$caseLoads;lengths_m=@(4.0,4.25,4.5)}
}
...
try {
    ...
    $patterns=Get-FixtureObject LoadPatterns
    foreach($caseName in $caseLoads.Keys) { $null=Invoke-Fixture cLoadPatterns $patterns Add @{Name=$caseName;MyType=1;SelfWTMultiplier=0.0;AddAnalysisCase=$true} }
    ...
    for($index=0;$index -lt $memberCount;$index++) {
        ...
        if($baselineDesign) {
            $null=Invoke-Fixture cPointObj $points SetRestraint @{Name=$ends.Point1;Value=[bool[]]@($true,$true,$true,$true,$false,$true);ItemType=0}
            $null=Invoke-Fixture cPointObj $points SetRestraint @{Name=$ends.Point2;Value=[bool[]]@($false,$true,$true,$true,$false,$true);ItemType=0}
        } else {
        ...
        $null=Invoke-Fixture cFrameObj $frames SetOutputStations @{Name=$name;MyType=2;MaxSegSize=0.0;MinSections=$stationCount;NoOutPutAndDesignAtElementEnds=$false;NoOutPutAndDesignAtPointLoads=$false;ItemType=0}
        foreach($caseName in $caseLoads.Keys) {
            $load=[double]$caseLoads[$caseName]+$(if($baselineDesign){0.0}else{0.125*($index%17)})
            $null=Invoke-Fixture cFrameObj $frames SetLoadDistributed @{Name=$name;LoadPat=$caseName;MyType=1;Dir=10;Dist1=0.0;Dist2=1.0;Val1=$load;Val2=$load;CSys='Global';RelDist=$true;Replace=$true;ItemType=0}
        }
    ...
    foreach($caseName in $caseLoads.Keys) { $null=Invoke-Fixture cAnalyze $analyze SetRunCaseFlag @{Name=$caseName;Run=$true;All=$false} }
    ...
    foreach($caseName in $caseLoads.Keys) { $null=Invoke-Fixture cAnalysisResultsSetup $setup SetCaseSelectedForOutput @{Name=$caseName;Selected=$true} }
    $rowCount=0
    foreach($name in $members) {
        $force=(Invoke-Fixture cAnalysisResults $results FrameForce @{Name=$name;ItemTypeElm=0}).outputs
        if($force.NumberResults -ne ($stationCount*$caseLoads.Count) -or @($force.Obj | Where-Object {$_ -ne $name}).Count -gt 0 -or @($force.LoadCase | Where-Object {$_ -notin $caseLoads.Keys}).Count -gt 0) {
            throw "Source member $name does not have the expected genuine same-object static rows."
        }
        if(@($force.ObjSta | Sort-Object -Unique).Count -ne $stationCount) { throw 'Output rows must represent the expected distinct actual source stations.' }
        $index=$members.IndexOf($name);$length=4.0+0.25*($index%9);$load=8.0+0.125*($index%17)
        if($baselineDesign) {
            foreach($caseName in $caseLoads.Keys) {
                $indices=@(0..($force.NumberResults-1) | Where-Object {$force.LoadCase[$_] -eq $caseName})
                if($indices.Count -ne $stationCount){throw 'Each baseline case must have all genuine source stations.'}
                foreach($row in $indices) {
                    $x=[double]$force.ObjSta[$row];$w=[double]$caseLoads[$caseName]
                    $expectedMoment=$w*$x*($length-$x)/2.0
                    # ETABS section-face V2 is negative at the I end for the
                    # positive local-2/up, gravity-loaded horizontal fixture.
                    $expectedShear=$w*($x-$length/2.0)
                    if([Math]::Abs([double]$force.M3[$row]-$expectedMoment) -gt 1e-7 -or [Math]::Abs([double]$force.V2[$row]-$expectedShear) -gt 1e-7 -or
                        [Math]::Abs([double]$force.P[$row]) -gt 1e-12 -or [Math]::Abs([double]$force.V3[$row]) -gt 1e-12 -or [Math]::Abs([double]$force.M2[$row]) -gt 1e-12 -or [Math]::Abs([double]$force.T[$row]) -gt 1e-12) {
                        throw 'Baseline source actions differ from the independently defined simply supported UDL solution.'
                    }
                }
            }
        } elseif([Math]::Abs($force.ObjSta[99]-$length) -gt 1e-8 -or
