...
public sealed class EtabsReflectionGetterHost : IEtabsGetterHost
{
    ...
    public static EtabsReflectionGetterHost AttachForces(EtabsHostExpectation expected)
    {
        var host = Attach(expected);
        try
        {
            ValidateMatrix(Assembly.LoadFrom(expected.ApiAssemblyPath), EtabsForceGetterMatrix.Allowed.Values);
            return host;
        }
        catch
        {
    ...
    public static EtabsReflectionGetterHost AttachGroup(EtabsHostExpectation expected)
    {
        var host = AttachBulk(expected);
        try { ValidateMatrix(Assembly.LoadFrom(expected.ApiAssemblyPath), EtabsGroupGetterMatrix.Allowed.Values); return host; }
        catch { host.Dispose(); throw; }
    }
    ...
    public static EtabsReflectionGetterHost AttachSource(EtabsHostExpectation expected)
    {
        ValidateMatrix(Assembly.LoadFrom(expected.ApiAssemblyPath), EtabsSourceGetterMatrix.Allowed.Values);
        return Attach(expected);
    }

    public static EtabsReflectionGetterHost AttachTopology(EtabsHostExpectation expected)
    {
        ValidateMatrix(Assembly.LoadFrom(expected.ApiAssemblyPath), EtabsTopologyGetterMatrix.Allowed.Values);
        return Attach(expected);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if ((!EtabsGetterMatrix.Allowed.TryGetValue(definition.Operation, out var frozen) || frozen != definition) &&
            ...
            (!EtabsSourceGetterMatrix.Allowed.TryGetValue(definition.Operation, out frozen) || frozen != definition) &&
            (!EtabsTopologyGetterMatrix.Allowed.TryGetValue(definition.Operation, out frozen) || frozen != definition))
