...
public static class BaselineInputMapper
{
    ...
    internal static BaselineMappingResult Map(BaselineSnapshotIndex index, BaselineProjectInputs inputs, string memberId)
        => Map(index, inputs, memberId, false);

    internal static BaselineMappingResult MapCore(BaselineSnapshotIndex index, BaselineProjectInputs inputs, string memberId)
        => Map(index, inputs, memberId, true);

    private static BaselineMappingResult Map(BaselineSnapshotIndex index, BaselineProjectInputs inputs, string memberId, bool coreOnly)
    {
        ...
        if (member is null || context is null || !coreOnly && !inputs.Project.ValuesAccepted) return Result(null, BaselineDesignState.NeedsInput, "INPUT.ACCEPTANCE", "Member context and explicitly accepted calculation inputs are required; professional approval remains a separate project fact.", "member_context,project");
        if (!coreOnly && (context.FireBasis is null || !Enum.IsDefined(context.FireBasis.Requirement) || context.FireBasis.Requirement == BaselineFireRequirement.Unspecified || !Text(context.FireBasis.DecisionReference)))
        ...
        if (!coreOnly && (context.FireBasis!.Requirement != BaselineFireRequirement.NotRequired || context.FireBasis.RequiredMinutes is not null))
        ...
        if (!coreOnly && !context.ScreeningPermitted) return Result(null, BaselineDesignState.NeedsInput, "DEFLECTION.METHOD_REQUIRED", "Explicit acceptance of span/depth screening is required; this profile does not calculate displacement.", "screening_permitted");
        if (!coreOnly && context.EffectiveSpanMm > 10000) return Result(null, BaselineDesignState.Unsupported, "DEFLECTION.SPAN_UNSUPPORTED", "The qualified screening method is limited to spans at most 10 m.", "effective_span_mm");
        ...
        var requiredRoles = coreOnly ? new[] { BaselineSelectionRole.Uls } : [BaselineSelectionRole.Uls, BaselineSelectionRole.SlsTotal, BaselineSelectionRole.SlsSustained];
        if (requiredRoles.Any(role => !rows.Any(x => roles[x.SelectionId] == role))) return Result(null, BaselineDesignState.NeedsInput, "ACTION.ROLE_COVERAGE", "The calculation's ULS/SLS source roles are incomplete.", "action_rows");
