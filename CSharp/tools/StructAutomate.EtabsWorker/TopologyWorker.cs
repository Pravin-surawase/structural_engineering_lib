...
internal static class TopologyWorker
{
    ...
    public static async Task<int> Run(string requestPath, string responsePath)
    {
        ...
        try
        {
            var request = JsonSerializer.Deserialize<Request>(await File.ReadAllBytesAsync(requestPath), options)
                ?? throw new InvalidDataException("Topology request is empty.");
            if (request.SchemaVersion != "structural.etabs_topology_request/v1" || request.Target is null ||
                ...
                throw new InvalidDataException("Topology request schema or target is invalid.");
            ...
            var handle = EtabsTopologyBroker.Start(new(request.RequestId, request.Target.ProcessId, request.DeadlineUtc, request.EvidencePath), request.Scope,
                () =>
                {
                    ...
                    return EtabsReflectionGetterHost.AttachTopology(expected);
                }, cancellation.Token);
            ...
            Console.WriteLine($"Topology qualification {result.State}; quiescent=true; evidence={result.FileSha256 ?? "none"}");
